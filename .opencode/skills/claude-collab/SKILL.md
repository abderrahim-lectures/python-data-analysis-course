...
**Localized project slugs (2026-09-11, coordinated):** the locale project
`[...slug]` pages emit BOTH a localized slug (canonical, e.g.
`/ar/مشاريع/كتالوج-البيانات`) and the English slug (alias) for each project.
Keep the dual-`params` shape; do not revert to plain English slugs. Slug text
comes from `src/lib/projectSlugs.data.ts` (generated by
`scripts/generate-project-slugs.cjs` from each locale's `title`), the sitemap
filter in `astro.config.mjs` drops alias URLs, `ProjectDetail.astro` needs
`canonicalPath` on `<Base>`, and hrefs must go through
