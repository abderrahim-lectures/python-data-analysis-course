...
|---|---|---|---|
| ![Homepage](./static/img/screenshots/homepage.png) | ![Lesson page with runnable cells](./static/img/screenshots/lesson-page.png) | ![Progress page with badges](./static/img/screenshots/progress-page.png) | ![Mobile lesson view](./static/img/screenshots/mobile-lesson.png) |
...
Every lesson's runnable cells execute in-browser via Pyodide, vendored under `public/pyodide/` by `scripts/vendor-pyodide.mjs` during `npm run build` and served from the site's own origin (no runtime CDN). It loads lazily on a cell's first Run click. Lesson notebooks under `notebooks/` are generated by `scripts/generate-notebooks.mjs` and committed; translated lessons keep their own notebooks at `notebooks/<ar|es|fr>/<section>/<track>-<slug>.ipynb` (also committed), and each locale's lesson page links only to its own notebook. `public/datasets/index.json` is the manifest that maps the dataset names lessons reference to the CSV files bundled under `public/datasets/`.
