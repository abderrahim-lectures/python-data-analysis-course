...
- **Languages:** English (default) plus Arabic, Spanish, and French via Docusaurus's built-in i18n system — see Internationalization section below for what's fully written now vs. scaffolded
...
- **A capstone bonus at the very end of the whole course**: install Python locally for real (graduating off the in-browser playgrounds) and build a first AI agent with LangChain's `deepagents`, using a free-tier AI API (Gemini's free API is the suggested default; any free API works). This is teased on the welcome page's learning-objectives summary from day one as motivation to finish, not sprung on students at the end — see the dedicated Capstone Bonus section below.
...
├── i18n/
│   ├── ar/                     # Arabic (RTL) — UI strings now, content translation scaffolded
│   ├── es/                     # Spanish — UI strings now, content translation scaffolded
│   └── fr/                     # French — UI strings now, content translation scaffolded
...
- **Content translation scope for this pass:** writing full lesson content for all 10 weeks × 2 tracks was already the agreed scope for English. Fully translating all of that into Arabic, Spanish, *and* French in the same pass roughly 4×s the content-writing work. Recommendation: build the complete i18n infrastructure now (locale switcher, RTL support, translated UI chrome) with **English as the fully-written source locale**, and scaffold `ar`/`es`/`fr` docs folders with the file structure in place so lesson content can be translated incrementally after — flag if full translated content for all locales is actually needed in this same pass, since that changes the size of this effort substantially.
- **This is intentional, not broken:** Docusaurus automatically falls back to the default-locale (English) copy of any doc page that has no translated version yet, so browsing the site in `/ar/`, `/es/`, or `/fr/` will show English lesson content mixed with translated UI chrome until content translation happens — worth knowing up front so it doesn't get mistaken for a bug during review. Future content translation work follows the same per-week issue → branch → PR flow as everything else (`type:i18n`, `area:*`).
...
- **What it covers**, in `docs/bonus/capstone-ai-agent.md`:
  ...
  2. Getting a **free-tier AI API key** (Gemini's free API via Google AI Studio is the suggested default; the lesson notes any free-tier API works the same way) and setting it as a local environment variable — never a key embedded in course content or committed to the repo.
...
- `docusaurus.config.js`: `projectName: 'python-data-analysis-course'`, `baseUrl: '/python-data-analysis-course/'`, `organizationName` set to whichever account `gh repo create` ends up under.
- **Known GH Pages footguns to get right up front**, since they're easy to miss until something 404s in production: `baseUrl` must match the repo name exactly for a project page (`/python-data-analysis-course/`, not `/`), and `trailingSlash` needs an explicit, consistent value (`false` is the common safe choice) — GH Pages' static-file serving behaves differently from `docusaurus start`'s dev server here, so this only actually gets exercised by the deployed site, not local `npm run serve`.
