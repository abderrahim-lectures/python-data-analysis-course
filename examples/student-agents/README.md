...
A gallery of agents students have built for the [Capstone Bonus](../../docs/bonus/capstone-ai-agent.md) — browse the folders below to see what other students built, or add your own.
...
Folders are added over time as students submit pull requests — just look through the subfolders in this directory on GitHub, or clone the repo and `ls examples/student-agents/`. Each one's `README.md` says what the agent's objective is and what tools it uses, so you can skim without running anything.
...
```bash
cp -r examples/student-agents/_template examples/student-agents/your-name
```
...
```bash
git add examples/student-agents/your-name
