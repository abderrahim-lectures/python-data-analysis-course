...
sidebar_label: "2026: بناء وكيل ذكاء اصطناعي"
slug: /bonus/2026-ai-agent
---

import CapstoneProgressCheckbox from '@site/src/components/CapstoneProgressCheckbox';

# 🎁 مشروع 2026 الختامي: انتقل إلى Python الحقيقية وابنِ وكيل ذكاء اصطناعي

**تهانينا على إنهاء الدورة.** كل شيء حتى الآن عمل في بيئة تجريبية معزولة داخل المتصفح — حتى تتمكن من البدء بكتابة Python من اليوم الأول بلا أي إعداد. هذا المشروع الختامي هو خطوة التخرّج: ثبّت Python فعليًا على جهازك الخاص، ثم استخدمها لبناء شيء لم تستطع بيئة البرمجة تشغيله أبدًا — وكيل ذكاء اصطناعي بمفتاح API خاص به، يستدعي نموذج لغوي حقيقي.

هذا اختياري وغير مُقيَّم. إنه موجود هنا لأن إنهاء 10 أسابيع من الأساسيات يستحق مكافأة تُشير إلى ما يأتي بعدها. وهو أيضًا مشروع هذا العام — كل عام يُضاف مشروع جديد، وتبقى مشاريع الأعوام السابقة متاحة؛ راجع [اختر مشروعك الختامي](/docs/bonus) للاطلاع على القائمة الكاملة.
...
**اختر أي مزوّد تفضله** — لا يتطلب أيٌّ منها بطاقة ائتمان وقت كتابة هذا النص، وهذه الدورة لا تفضّل واحدًا على آخر. الوكيل المثال في مستودع الدورة ([`examples/capstone-agent/`](https://github.com/abderrahim-lectures/python-data-analysis-course/tree/main/examples/capstone-agent)) يدعم الستة جميعًا جاهزين للاستخدام، ويُختار عبر إعداد واحد.
...
2. **الأكثر متانة**: لُف استدعاء `agent.invoke(...)` بـ `try`/`except` يلتقط الخطأ، وينتظر، ويعيد المحاولة تلقائيًا — تمامًا النمط المُدرَّس كمحتوى مكافأة سابقًا في الأسبوع 4 من Python 101. المثال الأوفى في المستودع يفعل هذا فعليًا: انظر `ask()` في [`examples/capstone-agent/agent.py`](https://github.com/abderrahim-lectures/python-data-analysis-course/tree/main/examples/capstone-agent/agent.py) لنسخة عاملة يمكنك نسخها، بما في ذلك تحليل مهلة إعادة المحاولة المقترحة من المزوّد من رسالة الخطأ.

:::tip[تستخدم مزوّدًا مختلفًا؟]
استبدل كتلة `ChatOpenAI(...)` بعميل مزوّدك الخاص — مثل `ChatGoogleGenerativeAI(model="gemini-3.5-flash", google_api_key=os.environ["GOOGLE_API_KEY"])` لـ Gemini، أو `ChatGroq(model="llama-3.3-70b-versatile", api_key=os.environ["GROQ_API_KEY"])` لـ Groq. كل شيء آخر في هذا الملف يبقى كما هو — `deepagents` لا يهتم بأي مزوّد يقف خلف النموذج. انظر [`examples/capstone-agent/agent.py`](https://github.com/abderrahim-lectures/python-data-analysis-course/tree/main/examples/capstone-agent) في مستودع الدورة لرؤية الستة جميعًا موصولين جنبًا إلى جنب، قابلين للاختيار بمتغيّر بيئة واحد.
:::
...
بنيت شيئًا تفتخر به؟ [`examples/student-agents/`](https://github.com/abderrahim-lectures/python-data-analysis-course/tree/main/examples/student-agents) معرض لوكلاء أرسلها طلاب آخرون — وملف README الخاص به يحتوي شرحًا كاملاً وودودًا للمبتدئين لإضافة وكيلك عبر **طلب سحب (pull request)**، حتى لو لم تستخدم git من قبل: عمل fork للمستودع، إنشاء فرع، تثبيت ملفاتك، وفتح طلب السحب، خطوة بخطوة. لا يُفترض أي خبرة سابقة بـ git.
