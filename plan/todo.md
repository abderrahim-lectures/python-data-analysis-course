...
- [x] **PageViews silent-failure fix** (decision-free half of the analytics item): the `sendBeacon` path posted to Supabase with **no apikey/Auth header** — beacons cannot set headers, so every row silently 401'd whether the boolean was true or false; the fetch fallback only ran on exception (never, since sendBeacon rarely throws). Replaced with a single `fetch(..., keepalive: true)` carrying `apikey`/`Authorization`/`Prefer: return=minimal`, plus a `console.warn` when the response is not `ok` so a misconfigured or RLS-blocked table is visible the moment analytics is wired (env currently unset → still a no-op). The design decision that remains for the user: whether LearnerActivity's fabricated presence/absent-count stays gated behind env or the GA/Supabase pieces get removed entirely.
