...
- ✅ Imprime exactamente 10 líneas, una por cada cita de la página principal.
- ✅ Cada línea impresa tiene texto real, un nombre de autor real, y una lista de etiquetas no vacía — no `None` ni strings vacíos.
...
- ✅ `uv run python scrape.py` termina e imprime una línea "Saved N quotes".
- ✅ `quotes.csv` existe y tiene más de 10 filas (es decir, realmente siguió la paginación, no solo la página principal).
- ✅ Abrir `quotes.csv` en un editor de texto muestra tres columnas — `text`, `author`, `tags` — sin filas obviamente rotas o vacías.
...
- ✅ `df["tags"]` contiene listas reales de Python después de la llamada a `.apply`, no strings — verifica con `type(df["tags"].iloc[0])`.
- ✅ `df["quote_length"]` es una columna numérica sin valores faltantes.
- ✅ `df.head()` muestra texto limpio sin espacios en blanco perdidos al inicio/final.
...
- ✅ `top_tags.png` y `quote_length_dist.png` existen ambos y se abren como imágenes reales.
- ✅ El eje x del gráfico de barras empieza en 0.
- ✅ Ambos gráficos tienen título y ejes etiquetados — sin números pelados sin unidades.
