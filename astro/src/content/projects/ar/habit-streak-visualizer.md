...
- ✅ تشغيل `checkin.py` مرتين لنفس العادة والتاريخ، مرة "y" ومرة "n"، يترك السجل بكلا الصفين — ستحتاج لتقرير (الخطوة التالية) أيهما يفوز.
- ✅ فتح `checkins.csv` في محرر نصوص يُظهر بالضبط ثلاثة أعمدة، صف واحد لكل تسجيل دخول، قابل للقراءة البشرية.
- ✅ يمكنك تسجيل تسجيل دخول لتاريخ ماضٍ بـ`--date` و`--done`، دون المُوجِّه التفاعلي.
...
- ✅ يحتوي `daily.index` كل يوم تقويمي بين أول وآخر مُدخَل سجل خاص بك، بلا فجوات — يطابق `len(daily)` عدد تلك الأيام بالضبط.
- ✅ عدّ سلسلة معروفة من أيام "y" متتالية في سجل اختبارك يدويًا يطابق ما يُبلِّغ عنه `compute_streaks` لـ`longest_streak`.
- ✅ تسجيل "n" (أو تخطي يوم) يُعيد `current_streak` إلى 0 المرة التالية التي تحسبها.
...
- ✅ `grid.shape[0]` هو بالضبط 7 (صف واحد لكل يوم أسبوع)، بغض النظر عن طول نطاق التاريخ.
- ✅ تغذية `build_grid` بنطاق تاريخ يعبر أول يناير *لا* ينتج مجموعتين من أعمدة أرقام أسابيع منخفضة — تزداد الأعمدة باطراد عبر الحد.
- ✅ أول وآخر بضع خلايا في الشبكة (قبل أول يوم مُسجَّل، بعد الأخير) هي `NaN`، لا `0`.
...
- ✅ تُظلِم الخريطة الحرارية المعروضة بوضوح عبر سلسلة حقيقية متعددة الأيام في بياناتك، بدلًا من أن تبدو كل خلية "منجَزة" متطابقة.
- ✅ تُعرَض الخلايا خارج نطاق تاريخك المُسجَّل برمادي مسطّح، قابل للتمييز بلمحة عن يوم "فائت" أزرق شاحب.
- ✅ تشغيل أداة التصور ضد بيانات العينة المُرفَقة ينتج شبكة يمكن التعرف على شكلها كرسم مساهمات GitHub: سبعة صفوف، أعمدة كثيرة، محور زمني واضح من اليسار لليمين.
