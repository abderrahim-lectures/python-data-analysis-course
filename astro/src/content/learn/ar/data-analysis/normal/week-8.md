...
track: normal
week: 8
description: "الأسبوع 8: تنظيف البيانات — data-analysis (normal track)."
---


# الأسبوع 8: تنظيف البيانات

<span class="gamified-flourish">🧹 مجموعات البيانات الحقيقية فوضوية. هذا الأسبوع عن جعل مجموعة بيانات جديرة بالثقة قبل تحليلها.</span>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> <code>df.isna().sum()</code> — رقم واحد لكل عمود، عدّ القيم المفقودة (NaN) في ذلك العمود.</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> أضف NaN يدويًا أولًا (مثلًا <code>df.loc[0, "quiz1"] = None</code>)، ثم قارن <code>df.dropna(subset=["quiz1"])</code> (صفوف أقل) مقابل <code>df["quiz1"].fillna(df["quiz1"].mean())</code> (نفس عدد الصفوف، القيمة المفقودة مُستبدَلة) لرؤية فرق عدد الصفوف مباشرة.</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> <code>df["name"].str.lower()</code> تُحوّل العمود كاملًا لأحرف صغيرة في استدعاء واحد، مُوجَّه — مكافئ للتكرار واستدعاء <code>.lower()</code> على كل اسم فرديًا، لكن دون كتابة الحلقة.</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> <code>df["name"].str.contains("a")</code> تُعيد قناعًا منطقيًا؛ غلّفها بـ<code>df[...]</code> للحصول على الصفوف المطابقة فقط، أو استدعِ <code>.sum()</code> على القناع لعدّها فقط.</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> كرّر صفًا أولًا (مثلًا pd.concat([df, df.iloc[[0]]]))، ثم شغّل .duplicated().sum() للتأكد من اكتشاف تكرار واحد على الأقل، و.drop_duplicates() لإزالته، مع فحص df.shape قبل وبعد للتأكد من انخفاض عدد الصفوف بواحد.</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> اكتب دالة مثل grade_letter تُعيد "Pass" إن كانت الدرجة &gt;= 60 و"Fail" خلاف ذلك، ثم طبّقها: df["quiz1_result"] = df["quiz1"].apply(pass_fail).</p>
...
<div class="quiz" data-quiz="data-analysis-normal-week-8">
      <div class="quiz-q" data-answer="1">
        <p class="quiz-q__prompt">1. أي دالة تعدّ القيم المفقودة لكل عمود؟</p>
      ...
      <div class="quiz-q" data-answer="1">
        <p class="quiz-q__prompt">2. ماذا تفعل pd.to_numeric(col, errors=&quot;coerce&quot;) بقيمة لا تستطيع تحويلها؟</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">ترفع استثناءً فورًا</button>
        <button class="quiz-q__opt" data-idx="1">تُحوّلها إلى NaN</button>
        <button class="quiz-q__opt" data-idx="2">تتركها كالنص الأصلي</button>
        <button class="quiz-q__opt" data-idx="3">تحذف ذلك الصف</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
      <div class="quiz-q" data-answer="1">
        <p class="quiz-q__prompt">3. لماذا نستخدم df[&quot;col&quot;].str.lower() بدلًا من df[&quot;col&quot;].lower()؟</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">متطابقتان، فقط أسلوب مختلف</button>
        <button class="quiz-q__opt" data-idx="1">.str مطلوبة لتطبيق دوال النصوص عنصرًا-بعنصر عبر Series</button>
        <button class="quiz-q__opt" data-idx="2">.lower() تعمل فقط على DataFrames، لا Series</button>
        <button class="quiz-q__opt" data-idx="3">.str.lower() أسرع للأعمدة الرقمية</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
      <div class="quiz-q" data-answer="1">
        <p class="quiz-q__prompt">4. تُزيل df.dropna(subset=[&quot;quiz1&quot;]) الصفوف حيث:</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">أي عمود مفقود قيمة</button>
        <button class="quiz-q__opt" data-idx="1">quiz1 تحديدًا مفقودة قيمة</button>
        <button class="quiz-q__opt" data-idx="2">quiz1 أقل من 60</button>
        <button class="quiz-q__opt" data-idx="3">فهرس الصف مفقود</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
      <div class="quiz-q" data-answer="1">
        <p class="quiz-q__prompt">5. .apply() على Series أفضل استخدام لها عندما:</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">تريد دائمًا أقصى أداء</button>
        <button class="quiz-q__opt" data-idx="1">لا توجد دالة موجَّهة مدمجة تفعل بالفعل ما تحتاجه</button>
        <button class="quiz-q__opt" data-idx="2">تعمل مع DataFrame، لا Series أبدًا</button>
        <button class="quiz-q__opt" data-idx="3">يحتوي العمود أرقامًا فقط</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
