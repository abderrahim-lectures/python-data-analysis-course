...
<span class="gamified-flourish">🎲 Une table de comptage de mots ressemble à de la comptabilité. C'est en réalité une distribution de probabilité déguisée.</span>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> Utilisez <code>sorted(counts, key=lambda w: counts[w], reverse=True)[:5]</code>, ou de façon équivalente triez les paires de <code>counts.items()</code> par comptage — le même motif de tri par clé que dans le programme récapitulatif de la semaine 5 du parcours normal de Python 101.</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> Parcourez <code>probs.values()</code> et sommez-les, ou plus simplement <code>sum(probs.values())</code> ; cela devrait afficher quelque chose d'extrêmement proche de 1.0 (comme 0.9999999999999999) à cause de l'arrondi en virgule flottante, et non exactement 1.0.</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> Les mots qui n'apparaissent qu'une fois auront chacun la même, plus petite, probabilité non nulle — calculable comme <code>1 / total</code>. Vous pouvez les trouver en filtrant <code>counts</code> pour les entrées où le comptage vaut 1.</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> Un mot jamais vu dans le corpus a un comptage de 0, il est donc simplement absent du dict <code>counts</code>/<code>probs</code> — y accéder directement avec <code>probs["giraffe"]</code> lève une <code>KeyError</code> plutôt que de retourner 0, c'est pourquoi <code>.get(word, 0)</code> compte ici aussi.</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> Utilisez directement collections.Counter : Counter(word for tokens in tokenized for word in tokens).most_common(5). Comparez son résultat à votre réponse du Défi 1 -- elles devraient lister les mêmes 5 mots dans le même ordre, simplement calculés avec un outil intégré plutôt qu'avec sorted() à la main.</p>
...
<div class="quiz" data-quiz="python-101-hard-week-2">
      <div class="quiz-q" data-answer="1">
        <p class="quiz-q__prompt">1. Pour une distribution de probabilité valide sur des mots, que doit valoir (approximativement) sum(probs.values()) ?</p>
        ...
        <button class="quiz-q__opt" data-idx="3">Le compte total de mots</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
      <div class="quiz-q" data-answer="2">
        <p class="quiz-q__prompt">2. Un modèle « unigramme » estime la probabilité d'un mot en se basant sur :</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">Les 2 mots précédents</button>
        <button class="quiz-q__opt" data-idx="1">La phrase entière jusqu'ici</button>
        <button class="quiz-q__opt" data-idx="2">Aucun contexte du tout — juste la fréquence globale</button>
        <button class="quiz-q__opt" data-idx="3">Le mot suivant seulement</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
      <div class="quiz-q" data-answer="2">
        <p class="quiz-q__prompt">3. Quelle probabilité le modèle unigramme attribue-t-il à un mot jamais vu dans le corpus ?</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">1.0</button>
        <button class="quiz-q__opt" data-idx="1">Un très petit nombre positif</button>
        <button class="quiz-q__opt" data-idx="2">0 (il est simplement absent de la table)</button>
        <button class="quiz-q__opt" data-idx="3">Cela lève une erreur au moment de la construction du modèle</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
      <div class="quiz-q" data-answer="1">
        <p class="quiz-q__prompt">4. counts.get(word, 0) + 1 est utilisé plutôt que counts[word] + 1 parce que :</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">Cela s'exécute plus rapidement</button>
        <button class="quiz-q__opt" data-idx="1">Cela évite une KeyError la première fois qu'un mot est vu</button>
        <button class="quiz-q__opt" data-idx="2">Cela ne fonctionne que pour les nombres</button>
        <button class="quiz-q__opt" data-idx="3">Ils se comportent de façon identique, c'est juste une question de style</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
      <div class="quiz-q" data-answer="2">
        <p class="quiz-q__prompt">5. La loi de Zipf décrit quel type de relation entre le rang d'un mot et sa fréquence ?</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">La fréquence augmente avec le rang</button>
        <button class="quiz-q__opt" data-idx="1">La fréquence est à peu près constante quel que soit le rang</button>
        <button class="quiz-q__opt" data-idx="2">La fréquence est à peu près inversement proportionnelle au rang</button>
        <button class="quiz-q__opt" data-idx="3">Il n'existe pas de relation générale</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
