...
<span class="gamified-flourish">🧹 Los datasets reales son desordenados. Esta semana se trata de hacer que un dataset sea confiable antes de analizarlo.</span>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> <code>df.isna().sum()</code> — un número por columna, el conteo de valores faltantes (NaN) en esa columna.</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> <code>df["name"].str.lower()</code> convierte toda la columna a minúsculas en una sola llamada, de forma vectorizada — equivalente a recorrerla con un bucle y llamar a <code>.lower()</code> en cada nombre individualmente, pero sin escribir el bucle.</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> <code>df["name"].str.contains("a")</code> devuelve una máscara booleana; envuélvela en <code>df[...]</code> para obtener solo las filas coincidentes, o llama a <code>.sum()</code> sobre la máscara para simplemente contarlas.</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> Duplica una fila primero (p. ej. pd.concat([df, df.iloc[[0]]])), luego ejecuta .duplicated().sum() para confirmar que se detecta al menos un duplicado, y .drop_duplicates() para eliminarlo, comprobando df.shape antes y después para confirmar que el conteo de filas bajó en uno.</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> Escribe una función como grade_letter que devuelva "Pass" si la calificación es &gt;= 60 y "Fail" en caso contrario, y luego aplícala: df["quiz1_result"] = df["quiz1"].apply(pass_fail).</p>
...
<div class="quiz" data-quiz="data-analysis-normal-week-8">
      <div class="quiz-q" data-answer="1">
        <p class="quiz-q__prompt">1. ¿Qué método cuenta los valores faltantes por columna?</p>
      ...
      <div class="quiz-q" data-answer="1">
        <p class="quiz-q__prompt">2. ¿Qué hace pd.to_numeric(col, errors=&quot;coerce&quot;) con un valor que no puede convertir?</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">Lanza una excepción inmediatamente</button>
        <button class="quiz-q__opt" data-idx="1">Lo convierte en NaN</button>
        <button class="quiz-q__opt" data-idx="2">Lo deja como el string original</button>
        <button class="quiz-q__opt" data-idx="3">Elimina esa fila</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
      <div class="quiz-q" data-answer="1">
        <p class="quiz-q__prompt">3. ¿Por qué usar df[&quot;col&quot;].str.lower() en lugar de df[&quot;col&quot;].lower()?</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">Son idénticos, solo es distinto estilo</button>
        <button class="quiz-q__opt" data-idx="1">.str es necesario para aplicar métodos de string elemento a elemento a través de una Series</button>
        <button class="quiz-q__opt" data-idx="2">.lower() solo funciona en DataFrames, no en Series</button>
        <button class="quiz-q__opt" data-idx="3">.str.lower() es más rápido para columnas numéricas</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
      <div class="quiz-q" data-answer="1">
        <p class="quiz-q__prompt">4. df.dropna(subset=[&quot;quiz1&quot;]) elimina filas donde:</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">A cualquier columna le falta un valor</button>
        <button class="quiz-q__opt" data-idx="1">Específicamente a quiz1 le falta un valor</button>
        <button class="quiz-q__opt" data-idx="2">quiz1 está por debajo de 60</button>
        <button class="quiz-q__opt" data-idx="3">Al índice de la fila le falta un valor</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
      <div class="quiz-q" data-answer="1">
        <p class="quiz-q__prompt">5. .apply() sobre una Series se usa mejor cuando:</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">Siempre quieres el máximo rendimiento</button>
        <button class="quiz-q__opt" data-idx="1">Ningún método vectorizado incorporado ya hace lo que necesitas</button>
        <button class="quiz-q__opt" data-idx="2">Estás trabajando con un DataFrame, nunca con una Series</button>
        <button class="quiz-q__opt" data-idx="3">La columna contiene solo números</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
