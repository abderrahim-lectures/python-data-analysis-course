...
<span class="gamified-flourish">🔗 La semana pasada: una variable a la vez. Esta semana: ¿cómo se mueven juntas dos (o más) variables?</span>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> <code>df["math_score"].corr(df["writing_score"])</code> — un único coeficiente de correlación de Pearson entre las dos columnas.</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> <code>df[["math_score","reading_score","writing_score"]].corr()</code> y luego <code>sns.heatmap(..., annot=True)</code> — la matriz por pares visualizada con color y números impresos juntos.</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> <code>sns.boxplot(data=df, x="lunch", y="math_score")</code> — un diagrama de caja agrupado que compara la distribución de math_score entre las dos categorías de lunch lado a lado.</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> Un tercer factor plausible es el estatus socioeconómico: podría afectar independientemente tanto al tipo de <code>lunch</code> (los programas de almuerzo gratis/reducido están ligados al ingreso del hogar) como a la preparación/recursos académicos (afectando las calificaciones) — así que el tipo de lunch y las calificaciones podrían correlacionarse sin que el lunch en sí cause las diferencias de calificación.</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> sns.scatterplot(data=df, x="math_score", y="reading_score", hue="test_preparation_course", alpha=0.6) -- colorea cada punto según el estado de preparación del examen, permitiéndote comparar visualmente si la relación matemáticas/lectura se ve similar para ambos grupos.</p>
...
<div class="quiz" data-quiz="data-analysis-hard-week-8">
      <div class="quiz-q" data-answer="2">
        <p class="quiz-q__prompt">1. Un coeficiente de correlación de Pearson de -0.9 indica:</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">Ninguna relación en absoluto</button>
        <button class="quiz-q__opt" data-idx="1">Una relación positiva fuerte</button>
        <button class="quiz-q__opt" data-idx="2">Una relación negativa fuerte</button>
        <button class="quiz-q__opt" data-idx="3">Que una variable causa la otra</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
      <div class="quiz-q" data-answer="1">
        <p class="quiz-q__prompt">2. El ejemplo de las ventas de helado / incidentes de ahogamiento ilustra:</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">Una relación causal perfecta</button>
        <button class="quiz-q__opt" data-idx="1">Correlación sin causalidad, ambas impulsadas por un tercer factor (el clima)</button>
        <button class="quiz-q__opt" data-idx="2">Una correlación negativa</button>
        <button class="quiz-q__opt" data-idx="3">Un error de lectura de gráfico, no un concepto estadístico</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
      <div class="quiz-q" data-answer="1">
        <p class="quiz-q__prompt">3. Un diagrama de caja agrupado (variable numérica dividida por categoría) muestra más que una media de groupby porque también revela:</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">Solo el tamaño exacto de la muestra</button>
        <button class="quiz-q__opt" data-idx="1">La dispersión/valores atípicos dentro de cada grupo, no solo el promedio</button>
        <button class="quiz-q__opt" data-idx="2">Causalidad entre las variables</button>
        <button class="quiz-q__opt" data-idx="3">Muestra exactamente la misma información que un gráfico de barras de medias</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
      <div class="quiz-q" data-answer="1">
        <p class="quiz-q__prompt">4. ¿Qué hace alpha=0.5 en un diagrama de dispersión de matplotlib?</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">Cambia la forma del marcador</button>
        <button class="quiz-q__opt" data-idx="1">Hace que los puntos sean semitransparentes para que los puntos superpuestos sean visibles</button>
        <button class="quiz-q__opt" data-idx="2">Filtra la mitad de los datos</button>
        <button class="quiz-q__opt" data-idx="3">Establece el umbral de correlación</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
      <div class="quiz-q" data-answer="1">
        <p class="quiz-q__prompt">5. pd.crosstab es la herramienta correcta para comparar:</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">Dos columnas numéricas, vía un coeficiente de correlación</button>
        <button class="quiz-q__opt" data-idx="1">Dos columnas categóricas, vía conteos de cada combinación</button>
        <button class="quiz-q__opt" data-idx="2">La distribución de una sola columna numérica</button>
        <button class="quiz-q__opt" data-idx="3">El orden de las filas en un DataFrame</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
