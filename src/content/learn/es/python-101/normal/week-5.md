...
track: normal
week: 5
description: "Lee y escribe archivos CSV en Python y construye un mini proyecto final usando el módulo csv."
---


# Semana 5: Leer y Escribir Archivos CSV

<span class="gamified-flourish">📊 Todo lo que aprendiste este mes construía hacia una sola cosa: los datos reales viven en archivos, no en código que escribiste a mano.</span>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> Recorre las filas del DictReader, mantén un par <code>best_name</code>/<code>best_score</code> corriente, y actualízalos cada vez que veas una calificación más alta — el mismo patrón de "seguir el máximo corriente" que encontrar el máximo de una lista.</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> Usa <code>csv.writer</code>, escribe una fila de encabezado, y luego recorre tu diccionario de promedios escribiendo una fila por estudiante — reflejando el lado de la lectura pero con <code>writerow</code> en lugar de iterar un reader.</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> Envuelve la conversión con int(...) para que un valor faltante/en blanco se omita o se le asigne un valor por defecto (por ejemplo, tratarlo como 0, o excluir ese quiz del promedio) en lugar de hacer que todo el programa se caiga — esta es exactamente el tipo de situación para la que está diseñado try/except (el bono de esta semana).</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> Reescribe el paso de escritura usando <code>csv.DictWriter</code> con <code>fieldnames=["name", "average"]</code>, llama a <code>writer.writeheader()</code> una vez, y luego <code>writer.writerow(...)</code> con un diccionario por estudiante dentro del bucle, en lugar de las filas de lista simples de <code>csv.writer</code>.</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> Usa el modo "a" (append, agregar) en lugar de "w": with open("summary.csv", "a", newline="") as f — esto añade filas nuevas después de lo que ya contiene el archivo, en lugar de borrarlo primero. Ten en cuenta que solo llamarías a writer.writeheader() la primera vez que se crea el archivo, no en cada agregado.</p>
...
<div class="quiz" data-quiz="python-101-normal-week-5">
      <div class="quiz-q" data-answer="2">
        <p class="quiz-q__prompt">1. ¿Qué tipo tiene todo valor leído de un archivo CSV con el módulo csv, antes de convertirlo?</p>
        ...
        <button class="quiz-q__opt" data-idx="3">Depende de la columna</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
      <div class="quiz-q" data-answer="1">
        <p class="quiz-q__prompt">2. ¿Qué te entrega csv.DictReader para cada fila, comparado con csv.reader?</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">Una lista de strings, igual que csv.reader</button>
        <button class="quiz-q__opt" data-idx="1">Un dict indexado por los nombres de columna del encabezado</button>
        <button class="quiz-q__opt" data-idx="2">Una tupla de ints</button>
        <button class="quiz-q__opt" data-idx="3">Un único string con toda la fila</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
      <div class="quiz-q" data-answer="1">
        <p class="quiz-q__prompt">3. En sorted(names, key=lambda n: averages[n]), ¿para qué sirve el argumento key?</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">Filtra algunos nombres</button>
        <button class="quiz-q__opt" data-idx="1">Le dice a sorted() según qué valor ordenar cada elemento</button>
        <button class="quiz-q__opt" data-idx="2">Convierte los nombres a números de forma permanente</button>
        <button class="quiz-q__opt" data-idx="3">Invierte el orden</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
      <div class="quiz-q" data-answer="2">
        <p class="quiz-q__prompt">4. ¿Qué módulo de la biblioteca estándar de Python se usó esta semana para leer/escribir archivos CSV?</p>
        <div class="quiz-q__options">
      ...
      <div class="quiz-q" data-answer="2">
        <p class="quiz-q__prompt">5. Abrir un archivo con open(&quot;data.csv&quot;, &quot;w&quot;) cuando el archivo ya existe:</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">Se niega a abrir, lanzando un error</button>
        <button class="quiz-q__opt" data-idx="1">Agrega contenido nuevo al final</button>
        <button class="quiz-q__opt" data-idx="2">Sobrescribe por completo el contenido existente</button>
        <button class="quiz-q__opt" data-idx="3">Lo abre en modo de solo lectura</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
