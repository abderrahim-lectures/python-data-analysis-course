...
<span class="gamified-flourish">⏱️ Esta es la semana en que toda la restricción de "Python puro, sin numpy" deja de ser un inconveniente y empieza a ser el punto central.</span>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> Envuelve la llamada a <code>generate_text</code> del CLI en un bucle que lea <code>input("Start word: ")</code>, y usa <code>input("Max words: ")</code> convertido con <code>int(...)</code> — el mismo patrón de entrada-y-luego-conversión de la Semana 1 del track Normal de Python 101, ahora alimentando un pipeline de generación en lugar de un cálculo simple.</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> Llama a <code>generate_text</code> (usando el <code>sample_next</code> consciente de la temperatura) tres veces cada una con <code>temperature=0.5</code> y <code>temperature=2.0</code> con la misma palabra inicial, y compara: las ejecuciones de baja temperatura deberían verse más repetitivas/predecibles, las de alta temperatura más erráticas.</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> A temperatura exactamente 1.0, <code>apply_temperature</code> eleva cada probabilidad a la potencia <code>1/1 = 1</code>, es decir, las deja sin cambios, y luego renormaliza dividiendo entre su suma (ya 1) -- así que matemáticamente es una operación nula, que es exactamente por qué <code>sample_next</code> trata como caso especial <code>temperature != 1.0</code> para saltarse el cómputo (redundante) por completo.</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> Divide el tiempo transcurrido a 1000x entre el tiempo transcurrido a 50x, y por separado divide 1000 entre 50 (=20). Si el algoritmo es verdaderamente lineal, la razón de tiempos debería estar aproximadamente cerca de 20 también -- confirmando que 20 veces más oraciones toma aproximadamente 20 veces más tiempo, no 400 veces (cuadrático) ni apenas más tiempo (constante).</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> Escribe generate_batch(probs_table, start_word, temperatures, max_words=10) que recorra con un bucle la lista temperatures, llamando a generate_text (o la versión consciente de la temperatura) una vez por valor y acumulando los resultados en una lista, p. ej. [generate_text(...) for t in temperatures] con el parámetro temperature de sample_next enhebrado a través.</p>
...
<div class="quiz" data-quiz="python-101-hard-week-5">
      <div class="quiz-q" data-answer="1">
        <p class="quiz-q__prompt">1. Una temperatura por debajo de 1.0 (p. ej. 0.5) hace que el texto generado sea:</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">Más aleatorio/caótico</button>
        <button class="quiz-q__opt" data-idx="1">Más predecible/repetitivo, más cercano a voraz</button>
        <button class="quiz-q__opt" data-idx="2">Exactamente idéntico a temperatura 1.0</button>
        <button class="quiz-q__opt" data-idx="3">Imposible de generar (lanza un error)</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
      <div class="quiz-q" data-answer="2">
        <p class="quiz-q__prompt">2. ¿Qué hace dict.setdefault(key, {}) si key ya está presente?</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">Sobrescribe el valor existente con {}</button>
        <button class="quiz-q__opt" data-idx="1">Lanza un KeyError</button>
        <button class="quiz-q__opt" data-idx="2">Deja el valor existente sin cambios</button>
        <button class="quiz-q__opt" data-idx="3">Elimina la clave</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
      <div class="quiz-q" data-answer="1">
        <p class="quiz-q__prompt">3. El ejercicio de cronometraje de esta semana en un corpus más grande está pensado para demostrar:</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">Que Python siempre es demasiado lento para ser útil</button>
        <button class="quiz-q__opt" data-idx="1">Una motivación concreta y sentida de por qué existen herramientas vectorizadas como numpy/pandas</button>
        <button class="quiz-q__opt" data-idx="2">Que los modelos bigrama son más rápidos que los modelos unigrama</button>
        <button class="quiz-q__opt" data-idx="3">Cómo arreglar código lento usando multihilos</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
      <div class="quiz-q" data-answer="1">
        <p class="quiz-q__prompt">4. if __name__ == &quot;__main__&quot;: se usa para:</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">Declarar la clase principal del programa</button>
        <button class="quiz-q__opt" data-idx="1">Marcar código que debería ejecutarse solo cuando el archivo se ejecuta directamente</button>
        <button class="quiz-q__opt" data-idx="2">Importar la biblioteca estándar</button>
        <button class="quiz-q__opt" data-idx="3">Acelerar el script</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
      <div class="quiz-q" data-answer="1">
        <p class="quiz-q__prompt">5. Duplicar aproximadamente el tamaño del corpus y ver que el tiempo de construcción se duplica aproximadamente es un ejemplo de:</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">Escalado en tiempo constante</button>
        <button class="quiz-q__opt" data-idx="1">Escalado en tiempo lineal</button>
        <button class="quiz-q__opt" data-idx="2">Escalado en tiempo cuadrático</button>
        <button class="quiz-q__opt" data-idx="3">Ninguna relación en absoluto</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
