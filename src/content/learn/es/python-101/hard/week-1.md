...
Este track trabaja con [`slm-corpus.csv`](/datasets/slm-corpus.csv), un pequeño conjunto de oraciones simples escritas a mano, una por fila bajo una columna `sentence`. Un "corpus" es simplemente el conjunto de datos de texto del que aprende un modelo de lenguaje — el nuestro es deliberadamente diminuto (20 oraciones) para que cada paso siga siendo lo bastante rápido de ejecutar e inspeccionar a mano en esta etapa temprana del curso.
...
<p class="challenge__answer">💡 <strong>Answer:</strong> Llama a <code>load_corpus</code>, y luego usa una comprensión de lista: <code>[tokenize(s) for s in corpus]</code>. Esto produce una lista de listas — cada lista interna son los tokens de una oración.</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> Recorre las oraciones tokenizadas con un bucle y toma <code>len(tokens)</code> para cada una, luego usa <code>sum(...) / len(...)</code> sobre todas ellas — el mismo patrón de promedio de las Semanas 4/5 del track Normal de Python 101, solo que aplicado a conteos de tokens en lugar de calificaciones.</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> Construye un <code>set()</code> y agrega cada token de cada oración tokenizada a él (o usa una comprensión de conjunto sobre una lista aplanada); <code>len(...)</code> de ese conjunto es el tamaño del vocabulario — el conteo de palabras *distintas*, a diferencia del conteo total de palabras, que cuenta las repeticiones.</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> Un tokenizador ingenuo con <code>.split()</code> trataría <code>"mat."</code> y <code>"mat"</code> como tokens diferentes, y <code>"Amina's"</code> quedaría pegado a su apóstrofo — la puntuación adjunta a una palabra no se elimina. Los tokenizadores reales (y el corpus de este curso) evitan esto manteniendo las oraciones libres de puntuación por ahora.</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> Llama a <code>tokenize_robust("The cat sat on the mat.")</code> y compáralo con <code>tokenize("The cat sat on the mat.")</code> — la versión robusta elimina el punto final antes de dividir, produciendo <code>"mat"</code> en lugar de <code>"mat."</code> como último token.</p>
...
<div class="quiz" data-quiz="python-101-hard-week-1">
      <div class="quiz-q" data-answer="1">
        <p class="quiz-q__prompt">1. ¿Cuál de las siguientes estima un modelo de lenguaje?</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">La siguiente palabra exacta con 100% de certeza</button>
        <button class="quiz-q__opt" data-idx="1">La probabilidad de la siguiente palabra dadas las palabras anteriores</button>
        <button class="quiz-q__opt" data-idx="2">Si una oración es gramaticalmente correcta</button>
        <button class="quiz-q__opt" data-idx="3">El número total de palabras en un documento</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
      <div class="quiz-q" data-answer="2">
        <p class="quiz-q__prompt">2. ¿Por qué tokenize() llama a .lower() antes de .split()?</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">Hace que el código se ejecute más rápido</button>
        <button class="quiz-q__opt" data-idx="1">Asegura que csv.DictReader funcione correctamente</button>
        <button class="quiz-q__opt" data-idx="2">Para que &quot;The&quot; y &quot;the&quot; se cuenten después como la misma palabra</button>
        <button class="quiz-q__opt" data-idx="3">Lo requiere la sintaxis de Python</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
      <div class="quiz-q" data-answer="1">
        <p class="quiz-q__prompt">3. ¿Qué devuelve load_corpus()?</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">Un único string largo</button>
        <button class="quiz-q__opt" data-idx="1">Una lista de strings, uno por oración</button>
        <button class="quiz-q__opt" data-idx="2">Un dict que asocia palabras con conteos</button>
        <button class="quiz-q__opt" data-idx="3">Un objeto de archivo CSV</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
      <div class="quiz-q" data-answer="2">
        <p class="quiz-q__prompt">4. ¿Qué significa &quot;tamaño del vocabulario&quot; para un corpus?</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">El conteo total de palabras incluyendo repeticiones</button>
        <button class="quiz-q__opt" data-idx="1">El número de oraciones</button>
        <button class="quiz-q__opt" data-idx="2">El número de palabras distintas</button>
        <button class="quiz-q__opt" data-idx="3">La longitud promedio de las oraciones</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
      <div class="quiz-q" data-answer="1">
        <p class="quiz-q__prompt">5. ¿Qué logra string.punctuation combinado con str.translate en tokenize_robust?</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">Convierte el texto a mayúsculas</button>
        <button class="quiz-q__opt" data-idx="1">Elimina los caracteres de puntuación antes de dividir</button>
        <button class="quiz-q__opt" data-idx="2">Cuenta cuántos signos de puntuación están presentes</button>
        <button class="quiz-q__opt" data-idx="3">Traduce la oración a otro idioma</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
