...
track: normal
week: 5
description: "الأسبوع 5: العمل مع ملفات CSV — python-101 (normal track)."
---


# الأسبوع 5: قراءة وكتابة ملفات CSV

<span class="gamified-flourish">📊 كل ما تعلمته هذا الشهر كان يبني نحو شيء واحد: البيانات الحقيقية تعيش في ملفات، لا في كود كتبته يدويًا.</span>
...
ملف CSV (قيم مفصولة بفواصل) هو جدول نصي بسيط — سطر واحد لكل صف، والقيم مفصولة بفواصل. مجموعة بيانات هذا الأسبوع، [`students-normal.csv`](/datasets/students-normal.csv)، تحتوي على صف رأس (`name,quiz1,quiz2,quiz3`) متبوعًا بصف لكل طالب.
...
<p class="challenge__answer">💡 <strong>Answer:</strong> افتح الملف بـ <code>csv.DictReader</code>، كرّر على الصفوف، حوّل عمود الدرجة بـ <code>int(...)</code>، واحتفظ بـ <code>total</code> و<code>count</code> جاريين (أو استخدم قائمة وsum()/len()) لحساب معدل الصف.</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> كرّر على صفوف DictReader، احتفظ بزوج <code>best_name</code>/<code>best_score</code> جارٍ، وحدّثهما كلما رأيت درجة أعلى — نفس نمط "تتبّع الحد الأقصى الجاري" لإيجاد أكبر عنصر في قائمة.</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> استخدم <code>csv.writer</code>، اكتب صف رأس، ثم كرّر على قاموس المعدلات كاتبًا صفًا لكل طالب — يعكس جانب القراءة لكن بـ<code>writerow</code> بدلًا من التكرار على قارئ.</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> غلّف تحويل int(...) بحيث تُتخطى القيمة المفقودة/الفارغة أو تُعطى قيمة افتراضية (مثلًا تُعامل كـ 0، أو يُستبعد ذلك الاختبار من المعدل) بدلًا من تعطيل البرنامج بالكامل — هذا بالضبط نوع الموقف الذي صُمم من أجله try/except (إضافي هذا الأسبوع).</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> أعد كتابة خطوة الكتابة باستخدام <code>csv.DictWriter</code> بـ <code>fieldnames=["name", "average"]</code>، استدع <code>writer.writeheader()</code> مرة، ثم <code>writer.writerow(...)</code> بقاموس لكل طالب داخل الحلقة، بدلًا من صفوف <code>csv.writer</code> العادية.</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> استخدم وضع "a" (إلحاق) بدلًا من "w": with open("summary.csv", "a", newline="") as f — يضيف هذا صفوفًا جديدة بعد أي محتوى موجود بالفعل بالملف، بدلًا من محوه أولًا. لاحظ أنك ستستدعي writer.writeheader() فقط في المرة الأولى التي يُنشأ فيها الملف، وليس مع كل إلحاق.</p>
...
<div class="quiz" data-quiz="python-101-normal-week-5">
      <div class="quiz-q" data-answer="2">
        <p class="quiz-q__prompt">1. ما نوع كل قيمة تُقرأ من ملف CSV بوحدة csv، قبل تحويلها؟</p>
        ...
        <button class="quiz-q__opt" data-idx="3">يعتمد على العمود</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
      <div class="quiz-q" data-answer="1">
        <p class="quiz-q__prompt">2. ماذا يمنحك csv.DictReader لكل صف، مقارنة بـ csv.reader؟</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">قائمة نصوص، مثل csv.reader تمامًا</button>
        <button class="quiz-q__opt" data-idx="1">dict مفتاحه أسماء أعمدة الرأس</button>
        <button class="quiz-q__opt" data-idx="2">صف (tuple) من أعداد صحيحة</button>
        <button class="quiz-q__opt" data-idx="3">نص واحد للصف كاملًا</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
      <div class="quiz-q" data-answer="1">
        <p class="quiz-q__prompt">3. في sorted(names, key=lambda n: averages[n])، ما دور وسيطة key؟</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">تُصفّي بعض الأسماء</button>
        <button class="quiz-q__opt" data-idx="1">تخبر sorted() بأي قيمة يرتب كل عنصر</button>
        <button class="quiz-q__opt" data-idx="2">تحوّل الأسماء إلى أرقام بشكل دائم</button>
        <button class="quiz-q__opt" data-idx="3">تعكس الترتيب</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
      <div class="quiz-q" data-answer="2">
        <p class="quiz-q__prompt">4. أي وحدة من مكتبة بايثون القياسية استُخدمت لقراءة/كتابة ملفات CSV هذا الأسبوع؟</p>
        <div class="quiz-q__options">
      ...
      <div class="quiz-q" data-answer="2">
        <p class="quiz-q__prompt">5. فتح ملف بـ open(&quot;data.csv&quot;, &quot;w&quot;) عندما يكون الملف موجودًا بالفعل سوف:</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">يرفض الفتح، ويرفع خطأ</button>
        <button class="quiz-q__opt" data-idx="1">يُلحق محتوى جديدًا في النهاية</button>
        <button class="quiz-q__opt" data-idx="2">يستبدل المحتوى الموجود بالكامل</button>
        <button class="quiz-q__opt" data-idx="3">يفتحه للقراءة فقط</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
