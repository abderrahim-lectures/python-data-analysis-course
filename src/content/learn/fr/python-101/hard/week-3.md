...
<span class="gamified-flourish">🔗 La semaine dernière : « quel mot est probable, tout court ? » Cette semaine : « quel mot est probable *juste après celui-ci* ? »</span>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> Construisez la table de bigrammes complète avec <code>bigram_counts</code> puis <code>bigram_probabilities</code>, et consultez directement <code>probs_table["the"]["cat"]</code> et <code>probs_table["the"]["dog"]</code>. Celui qui a la probabilité la plus grande est le plus susceptible de suivre « the » dans ce corpus.</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> Parcourez <code>probs_table</code> et affichez <code>len(probs_table[word])</code> pour chacun, ou utilisez <code>max(probs_table, key=lambda w: len(probs_table[w]))</code> pour trouver directement le mot avec le plus de successeurs distincts.</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> Le dernier mot d'une phrase n'apparaît jamais comme <code>premier</code> élément d'un bigramme (seulement comme <code>second</code>), il est donc absent en tant que clé de premier niveau dans <code>probs_table</code>, à moins qu'il n'apparaisse aussi en milieu de phrase ailleurs dans le corpus. Y accéder directement, par ex. <code>probs_table["mat"]</code>, lèverait une <code>KeyError</code> si « mat » ne démarre jamais de bigramme nulle part dans le corpus.</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> Étendez <code>bigrams</code> en une fonction générale <code>ngrams(tokens, n)</code> qui retourne des tuples de <code>n</code> tokens consécutifs en utilisant <code>range(len(tokens) - n + 1)</code> et le découpage, par ex. <code>tuple(tokens[i:i+n])</code>. Les bigrammes ne sont que le cas particulier n=2.</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> Utilisez l'assistant next_word_distribution : appelez-le avec un mot dont vous savez qu'il n'est jamais une clé de premier niveau (par ex. un mot qui ne fait que terminer des phrases), et confirmez qu'il retourne None au lieu de lever une KeyError, puis gérez explicitement ce cas None partout où vous l'appelez.</p>
...
<p class="challenge__answer">💡 <strong>Answer:</strong> Pour chaque clé de la table de comptages de bigrammes, sommez les valeurs de son dict interne pour obtenir le total des occurrences en bigramme de ce mot, et comparez-le au comptage de ce même mot dans le dict de comptages unigramme de la semaine 2 -- ils devraient correspondre exactement, puisque chaque occurrence d'un mot (sauf peut-être le tout dernier mot d'une phrase) démarre exactement un bigramme.</p>
...
<div class="quiz" data-quiz="python-101-hard-week-3">
      <div class="quiz-q" data-answer="1">
        <p class="quiz-q__prompt">1. Un bigramme est :</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">Un seul mot</button>
        <button class="quiz-q__opt" data-idx="1">Une paire de mots consécutifs</button>
        <button class="quiz-q__opt" data-idx="2">Le vocabulaire complet d'un corpus</button>
        <button class="quiz-q__opt" data-idx="3">Un seuil de probabilité</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
      <div class="quiz-q" data-answer="2">
        <p class="quiz-q__prompt">2. probs_table[&quot;the&quot;][&quot;cat&quot;] représente :</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">P(the)</button>
        <button class="quiz-q__opt" data-idx="1">P(cat)</button>
        <button class="quiz-q__opt" data-idx="2">P(cat | the) — la probabilité de &quot;cat&quot; sachant que le mot précédent était &quot;the&quot;</button>
        <button class="quiz-q__opt" data-idx="3">Le comptage total de &quot;the&quot; dans le corpus</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
      <div class="quiz-q" data-answer="1">
        <p class="quiz-q__prompt">3. À quoi somme (approximativement) chaque dict interne probs_table[word] ?</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">0</button>
        <button class="quiz-q__opt" data-idx="1">1</button>
        <button class="quiz-q__opt" data-idx="2">La taille du vocabulaire</button>
        <button class="quiz-q__opt" data-idx="3">Cela varie selon le mot, sans total fixe</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
      <div class="quiz-q" data-answer="1">
        <p class="quiz-q__prompt">4. Comparé à un modèle unigramme, un modèle bigramme :</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">Utilise moins de contexte (aucune mémoire du tout)</button>
        <button class="quiz-q__opt" data-idx="1">Utilise un mot de contexte précédent</button>
        <button class="quiz-q__opt" data-idx="2">Utilise la phrase entière comme contexte</button>
        <button class="quiz-q__opt" data-idx="3">Ne peut pas être représenté comme un dict</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
      <div class="quiz-q" data-answer="1">
        <p class="quiz-q__prompt">5. Un mot qui n'apparaît jamais comme PREMIER élément d'un quelconque bigramme dans le corpus est :</p>
        <div class="quiz-q__options">
        <button class="quiz-q__opt" data-idx="0">Automatiquement assigné une probabilité de 0 pour chaque mot suivant possible</button>
        <button class="quiz-q__opt" data-idx="1">Totalement absent en tant que clé de premier niveau dans probs_table</button>
        <button class="quiz-q__opt" data-idx="2">Tout de même inclus, avec une distribution vide {}</button>
        <button class="quiz-q__opt" data-idx="3">Impossible -- chaque mot démarre au moins un bigramme</button>
        </div>
        <p class="quiz-q__feedback" hidden></p>
      </div>
