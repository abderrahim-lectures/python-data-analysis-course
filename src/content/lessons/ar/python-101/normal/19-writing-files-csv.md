...
```python
open("file.txt", "w")   # كتابة (ينشئ الملف، ويُسطّح إن وُجد)
open("file.txt", "r")   # قراءة (افتراضي — تعمل الآن لأن الملف موجود)
open("file.txt", "a")   # إضافة (يُلحق في النهاية)
try:
    open("file.txt", "x")   # إنشاء — خطأ إن وُجد الملف، وهو موجود الآن
