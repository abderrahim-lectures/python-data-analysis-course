...
## الخطوة 1: تثبيت `uv`
### 1.1 `uv` أداة واحدة تحل محل سلسلة "ثبّت Python، ثم ثبّت pip، ثم ثبّت أداة بيئة افتراضية، ثم ثبّت...
...
### 1.3 أغلق طرفيتك وأعد فتحها، ثم تأكد من التثبيت:
...
### 2.1 ضع ملاحظاتك في مجلد `notes/` كملفات `.md` أو `.txt` عادية — ملاحظات محاضرات، يوميات، توثيق ك...
...
### 3.1 **التضمين** (embedding) هو قائمة من الأرقام — متجه — يمثّل *معنى* قطعة نص، لا صياغتها الدقيق...
...
- ✅ الرقم الأول من الشكل (shape) المطبوع يطابق عدد الأجزاء من الخطوة 2، والرقم الثاني هو 384.
...
### 4.1 لإيجاد الأجزاء ذات الصلة بسؤال ما، ضمّن السؤال بنفس النموذج، ثم رتّب كل جزء حسب مدى قرب متجه...
...
### 5.1 الاسترجاع وحده يعيد لك أجزاءً خامًا من ملاحظاتك الخاصة — مفيد، لكنه ليس إجابة مكتوبة. الخطوة...
...
### 5.2 تقرأ `python-dotenv` (المُثبَّتة في الخطوة 1) هذا الملف تلقائيًا إلى `os.environ`، وهو نفس ا...
...
استبدل كتلة `OpenAI(...)` بعميل مزوّدك الخاص، متبعًا نفس النمط الموضّح في [مشروع وكيل الذكاء الاصطناعي](/docs/projects/ai-agent#الخطوة-1-اكتب-وكيلك-الأول) — مثل حزمة `google-genai` من Google لـ Gemini، أو عميل `groq` الخاص لـ Groq. Cerebras وOpenRouter متوافقان أيضًا مع OpenAI، لذا تعمل حزمة `openai` معهما أيضًا، فقط بـ `base_url` مختلف.
:::
...
- ✅ يطبع `uv run python ask.py "سؤال حقيقي عن ملاحظاتك"` إجابة، لا تتبع خطأ (traceback).
