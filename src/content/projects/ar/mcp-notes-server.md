...
هذا يفترض Python 101 وارتياحًا في كتابة دوال بسيطة -- ويساعد كثيرًا أن تكون قد بنيت مشروع [بناء خادم MCP](/ar/مشاريع/mcp-server) أولًا، إذ يعيد هذا استخدام نفس نمط زخرفة `FastMCP` ويضيف فقط محتوى حقيقيًا للبحث فيه بدلًا من أداتين تجريبيتين. إنه اختياري وغير مُقيَّم؛ راجع [مشاريع من العالم الحقيقي](/ar/مشاريع) للاطلاع على القائمة الكاملة والنامية.
...
### 1.1 أنشئ مجلد `notes/` بجانب حيث سيعيش `server.py`، وضع حفنة من ملفات `.md` حقيقية فيه -- وصفة، ...
...
### 1.2 لا شيء هنا خاص بـMCP بعد -- إنه إدخال/إخراج ملفات عادي. هذا متعمَّد: اجعل الفهرسة تعمل بشكل ...
...
- ✅ يحتوي `notes/` على 4 ملاحظات `.md` حقيقية ومختلفة فعليًا على الأقل، كل واحدة بعنوان `# Title`.
- ✅ تُعيد `_all_notes()` `Note` واحدة لكل ملف، بالعنوان الصحيح مُستخرَجًا من كل عنوان.
- ✅ يشير `NOTES_DIR` إلى مجلد حقيقي موجود فعليًا على جهازك.
...
### 2.1 مع تحميل الملاحظات بشكل صحيح، اكتب الدوال التي تجيب فعليًا عن أسئلة حولها -- لا تزال بايثون ...
...
### 2.2 ترفض `get_note_by_title` عمدًا التخمين عندما يطابق عنوان جزئي أكثر من ملاحظة واحدة، بدلًا من...
...
- ✅ تجد `search_notes` كلمة مفتاحية تعرف أنها في إحدى ملاحظاتك، وتُعيد مقتطفًا حقيقيًا وصحيحًا.
- ✅ تُعيد `get_note_by_title` نص الملاحظة الكامل لعنوان دقيق، ورسالة حقيقية "كن أكثر تحديدًا" لعنوان جزئي غامض.
- ✅ تُعيد `list_recent_notes` الملاحظات بالترتيب الصحيح -- الأحدث تعديلًا أولًا.
...
### 3.1 كل شيء حتى الآن كان بايثون بسيطة. تحويله إلى خادم MCP هو زخرفة واحدة لكل دالة -- بلا كود على...
...
### 3.2 تفحص `@mcp.tool()` اسم كل دالة، ومعاملاتها المُلمَّح بأنواعها، وتوثيقها، وتبني تعريف أداة MC...
...
- ✅ يُعرِّف `server.py` الأدوات الثلاث بـ`@mcp.tool()` وتوثيقات حقيقية ومحددة.
- ✅ يبدأ `uv run mcp dev server.py` دون أخطاء ويسرد Inspector الأدوات الثلاث.
- ✅ استدعاء كل أداة يدويًا في Inspector يُعيد نفس النتائج الصحيحة التي رأيتها بالفعل في الخطوة 2.
...
### 4.1 يدعم المستوى المجاني لـ[Claude Desktop](https://claude.ai/download) الاتصال بخوادم MCP محلية...
...
- ✅ يظهر `notes` (أو اسم الخادم الذي اخترته) في قائمة أدوات/موصِّلات Claude Desktop بعد إعادة تشغيل كاملة.
- ✅ السؤال عن موضوع تعرف أنه في إحدى ملاحظاتك يُظهر فعليًا Claude يستدعي أداة، لا يجيب فقط من الذاكرة أو يخمّن.
- ✅ سؤال Claude سحب ملاحظة محددة بالاسم يُعيد محتواها الحقيقي الكامل.
