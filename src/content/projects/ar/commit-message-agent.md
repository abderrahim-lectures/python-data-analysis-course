...
هذا يفترض Python 101 وإلمامًا كافيًا بـgit لمعرفة ما يفعله `git add` و`git commit` — لا يُشترط أي شيء من تحليل البيانات. هذا اختياري وغير مُقيَّم؛ راجع [مشاريع من العالم الحقيقي](/ar/مشاريع) للاطلاع على القائمة الكاملة والنامية.
...
- ✅ `commit-message-agent/` موجود مع `pyproject.toml`، وحزمتا `openai` و`python-dotenv` مثبَّتتان.
- ✅ لديك مفتاح API حقيقي من مزوّد واحد، محفوظ في ملف `.env` في مجلد مشروعك — غير مُلصَق في أي سكربت.

## الخطوة 1: التقط فرق git في stage باستخدام `subprocess`
### 1.1 تُشغِّل وحدة `subprocess` في Python برنامجًا آخر وتلتقط مخرجاته كنص — هنا، ذلك البرنامج هو `...
...
### 1.2 `subprocess.run([...], capture_output=True, text=True)` هو السطر الأساسي: تمرير الأمر كـ**قا...
...
- ✅ تُعيد `get_diff_staged()` نص فرق حقيقي بعد تنفيذ `git add` على تغيير، وسلسلة فارغة عندما لا شيء في stage.
- ✅ تشغيل `commit_helper.py` داخل مجلد ليس مستودع git على الإطلاق يرفع `RuntimeError` واضحًا، لا تتبّعًا مُربِكًا من أعماق `subprocess`.
- ✅ يمكنك أن تشرح، بكلماتك الخاصة، لماذا تقرأ هذه الأداة `git diff --staged` بدلًا من `git diff` البسيط (التغييرات غير staged).
...
### 2.1 نموذج لغوي بلا تعليمات قد يكتب رسالة غامضة جدًا ("تحديث الكود")، أو مطوَّلة جدًا (فقرة كاملة...
...
- ✅ يمكنك أن تشرح، بكلماتك الخاصة، لماذا يحظر الـ prompt اختلاق رقم تذكرة أو مرجع issue غير موجود في الفرق.
- ✅ يحدد الـ prompt تنسيق مخرجات ملموسًا (`type(scope): summary`، جسم اختياري)، لا مجرد "اكتب رسالة التزام."
...
### 3.1 اربط كود التقاط الفرق من الخطوة 1 وsystem prompt من الخطوة 2 معًا، ثم أضف الجزء الذي يجعل هذ...
...
### 3.2 تُهم `truncate_diff` هنا أكثر مما قد يبدو للوهلة الأولى — انظر قسم المآزق أدناه لمعرفة لماذا...
...
- ✅ يطبع `uv run python commit_helper.py` مسودة بأسلوب Conventional Commits لفرق staged حقيقي.
- ✅ كتابة `r` عند المُوجِّه تسأل النموذج مجددًا وتطبع مسودة (ربما مختلفة)، دون فعل أي شيء آخر.
- ✅ كتابة `n` تلغي بنظافة، وكتابة `e` تتيح لك كتابة رسالة بديلة قبل المتابعة.
...
### 4.1 القطعة الأخيرة: استبدل العنصر النائب "(كنت سألتزم هنا...)" من الخطوة 3 بدالة تُشغِّل فعليًا ...
...
### 4.2 جرّب الحلقة الكاملة ضد تغيير حقيقي:
...
- ✅ كتابة `y` عند المُوجِّه تُنشئ فعليًا التزامًا حقيقيًا — يُظهر `git log -1` الرسالة التي قبلتها.
- ✅ كتابة `n` عند المُوجِّه تترك تغييراتك في stage محفوظة وغير مُثبَّتة — لم يحدث شيء.
- ✅ يمكنك الإشارة إلى سطر الكود الوحيد الذي يُستدعى فيه `git commit` فعليًا، وشرح لماذا يمكن الوصول إليه من مكان واحد بالضبط.
