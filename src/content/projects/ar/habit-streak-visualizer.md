...
### 1.1 السجل هو CSV بسيط بثلاثة أعمدة: `date`، و`habit`، و`done`. صف واحد لكل تسجيل دخول. ملف مسطّح...
...
### 1.2 يغلّف CLI صغير هذا بتفاعل "هل فعلتها اليوم؟ y/n":
...
### 2.1 السلسلة هي سلسلة من *أيام تقويمية متتالية* مُسجَّلة كمُنجَزة، بلا فجوة. القرار التصميمي المه...
...
### 2.2 `reindex` تقوم بالعمل الفعلي هنا: تأخذ `Series` بالتواريخ الموجودة فعليًا فقط وتوسّعها على *...
...
### 3.1 هذه هي اللحظة التعليمية الحقيقية للمشروع. رسم مساهمات GitHub هو شبكة: سبعة صفوف (واحد لكل يو...
...
### 4.1 يجب ألا تكون كثافة اللون ثنائية فقط (تم/لم يتم) — يجب أن يُقرَأ يوم هو الخامس عشر في صف من س...
...
### 4.2 غذِّ ذلك إلى `build_grid` مكان الملء البسيط 0/1، ثم اعرض بـmatplotlib — تدرج تسلسلي بلون واح...
...
### 4.3 النسخة الكاملة — بتسميات أشهر على طول المحور السيني وخطوط شبكة بين الخلايا — تعيش في [`examp...
