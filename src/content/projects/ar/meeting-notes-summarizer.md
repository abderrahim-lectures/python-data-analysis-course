...
### 1.1 أنشئ مجلد `transcripts/` وضع فيه نص اجتماع نصيًا عاديًا — أو انسخ واحدة من العيّنات الثلاث ا...
...
### 1.2 تحميله هو أصغر خطوة ممكنة، عمدًا:
...
- ✅ يطبع `uv run python load_transcript.py <path>` عدد أحرف غير صفري ومعاينة تبدو كنص اجتماع حقيقي.
- ✅ تشغيله على مسار غير موجود يثير خطأ Python واضحًا بدلًا من عدم فعل أي شيء بصمت.
- ✅ تشغيله على ملف فارغ يُثير `ValueError` الذي كتبته، لا خطأًا مربكًا في مرحلة لاحقة.
...
### 2.1 هذه هي المهارة الفعلية التي يعلّمها هذا المشروع: بدلًا من أن تطلب من نموذج ملخصًا حرّ الشكل ...
...
- ✅ تُعيد `build_prompt(transcript)` قائمة من قاموسي رسالتين (`system`، `user`)، مع نص الاجتماع مضمّنًا فعلًا في رسالة المستخدم.
- ✅ يمكنك الإشارة إلى الجملة الدقيقة في `JSON_SCHEMA_DESCRIPTION` التي تخبر النموذج ماذا يفعل عندما لا يُسمّى أي owner.
- ✅ تستطيع أن تشرح، في جملة واحدة، لماذا يُكتب المخطط كمثال JSON حرفي بدلًا من وصف فقرة.
...
### 3.1 الآن أرسل الـ prompt وحوّل أي نص يعود إلى بيانات Python حقيقية — `dict` تستطيع التكرار عليه،...
...
- ✅ يطبع `uv run python summarize.py transcripts/standup.txt` JSON صالحًا وقابلًا للقراءة بكل المفاتيح الثلاثة المطلوبة.
- ✅ تستطيع أن تشرح ماذا يفعل `extract_json` باستجابة ملفوفة في ```` ```json ... ``` ````، مقابل استجابة بلا أي سور كود.
- ✅ تغيير `REQUIRED_KEYS` مؤقتًا ليشمل مفتاحًا تعرف أنه ليس في المخطط وإعادة التشغيل يُنتج `ValueError` واضحًا خاصًا بك، لا انهيارًا في مكان آخر.
...
### 4.1 الـ `dict` المحلَّل هو بالضبط ما تريده للحفظ في قاعدة بيانات أو للتغذية إلى سكربت آخر، لكنه ...
...
- ✅ تُعيد `format_markdown(summary, "standup.txt")` سلسلة تبدأ بعنوان `# Meeting Summary`.
- ✅ عنصر عمل بلا owner مُسمّى يظهر كـ"unassigned"، لا فراغ أو الكلمة "None".
- ✅ تمرير ملخص تكون فيه كل قائمة فارغة ما زال يُنتج Markdown صالحًا وقابلًا للقراءة (أسطر `_No ... recorded._`)، لا قسمًا فارغًا أو مكسورًا.
...
### 5.1 اربط القطع معًا: حمّل نص اجتماع، واستدعِ النموذج، وحلّل JSON وتحقّق منه، ثم اكتب ملفي `.md` ...
...
- ✅ يطبع `uv run python summarize.py transcripts/standup.txt` ملخص Markdown قابلًا للقراءة ويُبلِّغ عن كتابة ملفي مخرجات.
- ✅ يوجد كل من `standup_summary.json` و`standup_summary.md` بعد ذلك، وملف JSON صالح (افتحه، أو أعد تحليله بـ`json.load`).
- ✅ تشغيله على نص اجتماع ثانٍ مختلف يُنتج ملخصًا يعكس فعلًا محتوى *ذلك* النص — لا نسخة من مخرجات الأول.
...
- ماذا سينكسر لو شغّلت هذا على نص اجتماع من ساعتين و15,000 كلمة بدلًا من هذه العيّنات القصيرة؟ عند أي نقطة ستحتاج إلى استراتيجية مثل نهج التقطيع من مشروع [RAG](/ar/مشاريع/rag-notes) بدلًا من إرسال كل شيء في prompt واحد؟
...
- ادمج هذا مع مشروع [وكيل الذكاء الاصطناعي](/ar/مشاريع/ai-agent): أعطِ الوكيل أداة تستدعي `summarize()` على ملف نص اجتماع، ليقدر هو أن يقرر *متى* يلخّص كجزء من مهمة أكبر بدلًا من أن تشغّل السكربت دائمًا يدويًا.
