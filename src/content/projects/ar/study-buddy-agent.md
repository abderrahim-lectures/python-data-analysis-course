...
هذا اختياري وغير مُقيَّم ، مناسب جيدًا بمجرد أن تنهي Python 101؛ لا شيء من تحليل البيانات مطلوب. راجع [مشاريع من العالم الحقيقي](/ar/مشاريع) للاطلاع على القائمة الكاملة والنامية.
...
يقرأ `python-dotenv` هذا الملف إلى `os.environ` تلقائيًا، نفس النمط المستخدم طوال مشروعي [وكيل الذكاء الاصطناعي](/ar/مشاريع/ai-agent) و[تطبيق RAG](/ar/مشاريع/rag-notes) لو نفّذت أيًّا منهما. مفتاح API سرّ، تمامًا مثل كلمة المرور ، أي شخص يملكه يمكنه استخدام حصة حسابك.
...
### 1.1 ضع ملف `.txt` أو `.md` من ملاحظات دراستك الخاصة في مكان ما داخل مشروعك ، مجلد `notes/`، نفس ...

**👟 تلميح البداية :**

ضع ملف `.txt` أو `.md` من ملاحظات دراستك الخاصة في مكان ما داخل مشروعك ، مجلد `notes/`، نفس اصطلاح [مشروع تطبيق RAG](/ar/مشاريع/rag-notes)، مكان معقول. قراءته ليست جديدة عليك:
...
- **الخيار B ، جزّئ، وضمّن، واسترجع**، تمامًا كما يفعل [مشروع تطبيق RAG](/ar/مشاريع/rag-notes): اقسّم ملاحظاتك إلى قطع صغيرة، وضمّنها محليًا، واسترجع فقط الأكثر صلة لكل سؤال. يتوسع هذا ليشمل مجلد ملاحظات بعشرات الملفات الطويلة التي لن تتسع أبدًا في prompt واحد.
...
### 2.1 اطلب من النموذج عددًا ثابتًا من الأسئلة، كل واحد مقترنًا بإجابة متوقعة ، وكن صريحًا في الـpr...
...
### 3.1 الآن الجزء الذي يجعل من هذا اختبارًا لا مجرد مولّد أسئلة: اطرح كل سؤال، اقرأ إجابة الطالب ال...
...
### 4.1 `run_quiz` أعلاه يتتبع `score` بالفعل أثناء تقدمه ويطبع سطر `score/total` نهائيًا بمجرد انته...
...
- بمجرد أن يتوقف ملف ملاحظات واحد عن الكفاية ، ملاحظات فصل دراسي كامل عبر ملفات كثيرة ، أعد استخدام خط أنابيب [مشروع تطبيق RAG](/ar/مشاريع/rag-notes) `prepare_notes.py`/`build_index.py`/`retrieve.py`: استرجع القطع الأكثر صلة لـ*موضوع* تريد أن تُختبر عليه، وأطعمها إلى `generate_questions` بدلًا من ملف واحد كامل.
