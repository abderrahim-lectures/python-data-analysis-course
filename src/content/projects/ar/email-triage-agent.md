...
[![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/abderrahim-lectures/python-data-analysis-course/blob/main/examples/email-triage-agent/notebook.ar.ipynb)
[![Open In Kaggle](https://kaggle.com/static/images/open-in-kaggle.svg)](https://kaggle.com/kernels/welcome?src=https://github.com/abderrahim-lectures/python-data-analysis-course/blob/main/examples/email-triage-agent/notebook.ar.ipynb)
[![Binder](https://mybinder.org/badge_logo.svg)](https://mybinder.org/v2/gh/abderrahim-lectures/python-data-analysis-course/main?filepath=examples%2Femail-triage-agent%2Fnotebook.ar.ipynb)
...
مفتاح API سرّ، تمامًا مثل كلمة المرور ، أي شخص يملكه يمكنه استخدام حصة حسابك. معاملته كمتغيّر بيئة بدلًا من نص ثابت مُضمَّن في الكود هي الممارسة القياسية لهذا السبب بالتحديد، وهو نفس عادة الأمان الواقعية المُعلَّمة في [مشروع وكيل الذكاء الاصطناعي](/ar/مشاريع/ai-agent).
...
### 1.1 يشحن مثال المستودع ستة رسائل بريد إلكتروني نموذجية قصيرة وواقعية في `sample_emails/` ، طلب ع...
...
### 1.2 انسخ الملفات النموذجية الستة من مجلد [`sample_emails/`](https://github.com/abderrahim-lectur...
...
- ✅ يعمل `uv run python triage.py` دون أخطاء ويطبع ست رسائل مُحمَّلة.
- ✅ يُظهر كل سطر مطبوع موضوعًا ومرسِلًا حقيقيين، لا `"unknown"` أو `"(no subject)"`.
- ✅ `sample_emails/` موجود في مجلد مشروعك ويحتوي الملفات الستة `.txt`.
...
### 2.1 الآن سلّم كل رسالة مُحلَّلة إلى نموذج لغوي واطلب منه فرزها إلى فئة وأولوية ، خطوة الفرز الفع...
...
### 2.2 حدِّث كتلة `if __name__ == "__main__":` لاستدعائها فعليًا:
...
- ✅ يطبع `uv run python triage.py` سطر فئة، وأولوية، وسبب لكل رسائل البريد النموذجية الست.
- ✅ يحصل بريد العميل العاجل والنشرة الإخبارية على فئتين وأولويتين مختلفتين بوضوح.
- ✅ لا `JSONDecodeError` ، إن رأيت واحدًا، اطبع سلسلة `content` الخام قبل تحليلها لترى ما أعاده النموذج فعليًا.
...
### 3.1 هذه هي الخطوة حيث يبدأ "الوكيل" بأن يعني أكثر من "مُصنِّف" ، لأي شيء وسمه النموذج بـ`needs_r...
...
- ✅ `draft_reply` مُعرَّفة، تأخذ `Email`، وتُعيد سلسلة نصية بسيطة ، لا شيء فيها يلمس الشبكة عدا استدعاء API واحد للنموذج اللغوي.
- ✅ يمكنك الإشارة إلى المكان الدقيق في كودك حيث سيلزم إرسال رد منه، وتأكيد أن ذلك الكود غير موجود.
- ✅ تفهم *لماذا* يهم هذا، لا فقط أنها قاعدة ، راجع الأسئلة السقراطية أدناه.
...
### 4.1 اربط كل شيء معًا ، صنّف كل رسالة، صُغ ردًّا للتي تحتاج واحدًا، واحفظ كل مسودة في مجلد محلي `...
...
- ✅ يعمل `uv run python triage.py` حتى الاكتمال ويطبع سطر فرز لكل رسائل البريد النموذجية الست.
- ✅ يحتوي `drafts/` ردًّا محفوظًا لكل رسالة وسمها النموذج بـ`needs_reply: true`، ولا ملف للتي لم يفعل.
- ✅ فتحت وقرأت فعليًا مسودة رد واحدة على الأقل، ويمكنك القول ما إذا كنت لترسلها كما هي أو تعدّلها أولًا.
...
- جرّب استدعاءً ثانيًا للنموذج اللغوي يراجع مسودة النموذج *الأول* قبل حفظها ، نمط بسيط من مرورين "صُغ، ثم انتقد،" وتذوّق أول لطيف لخطوط أنابيب وكيل متعددة الخطوات مثل تلك في [مشروع وكيل الذكاء الاصطناعي](/ar/مشاريع/ai-agent).
