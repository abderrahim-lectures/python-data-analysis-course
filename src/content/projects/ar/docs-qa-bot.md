...
يأخذ هذا المشروع خط أنابيب التوليد المعزَّز بالاسترجاع من [بناء تطبيق RAG](/ar/مشاريع/rag-notes) — تضمينات محلية، بحث تشابه جيب التمام بـNumPy، نموذج لغوي من مستوى مجاني للإجابة النهائية — ويضع عليه واجهة أمامية مختلفة: بدلًا من سكربت تُشغّله من طرفية سؤالًا واحدًا في كل مرة، يجيب نفس خط الأنابيب عن الأسئلة حيًّا، داخل خادم Discord، كلما ذكر أحدهم البوت. لا شيء يتعلق بـ*كيفية* الاسترجاع أو التوليد يتغير؛ فقط الواجهة تتغير.

هذا يفترض Python 101. يُنصَح بشدة ببناء [بناء تطبيق RAG](/ar/مشاريع/rag-notes) أولًا — يعيد هذا المشروع استخدام كود التضمين/الاسترجاع الخاص به مباشرة ويتجاوز بسرعة الأجزاء التي شرحها بالفعل بعمق.

هذا اختياري وغير مُقيَّم. راجع [مشاريع من العالم الحقيقي](/ar/مشاريع) للاطلاع على القائمة الكاملة والنامية.
...
كل ما علّمه [مشروع تطبيق RAG](/ar/مشاريع/rag-notes) عن معالجة مفاتيح API للنماذج اللغوية ينطبق هنا أيضًا، لسرّ ثانٍ: لا تُبرمج رمز البوت أبدًا كنص ثابت، لا ترفعه أبدًا، واحتفظ به في ملف `.env` محلي (أدناه) بدلًا من ذلك.
:::

### احصل على مفتاح API مجاني لنموذج لغوي

يحتاج نصف التوليد من خط الأنابيب هذا نفس نوع مفتاح النموذج اللغوي من مستوى مجاني مثل [مشروع تطبيق RAG](/ar/مشاريع/rag-notes) — **اختر المزوّد الذي تفضّله**، لا يتطلب أي منها بطاقة ائتمان وقت كتابة هذا:
...
- ✅ تطبيق وبوت Discord موجودان في بوابة المطورين، ونسخت رمزه.
- ✅ "Message Content" مُفعَّل تحت Privileged Gateway Intents.
- ✅ لديك مفتاح API لنموذج لغوي من مستوى مجاني من مزوّد اخترته.
- ✅ اكتمل `uv init`/`uv add` دون أخطاء، ولدى `.env` كل من `DISCORD_BOT_TOKEN` ومفتاح نموذجك اللغوي مضبوطين.
...
### 1.1 هذه الخطوة هي الخطوتان 2 و3 من مشروع تطبيق RAG، بلا تغيير في الجوهر، فقط موجَّهة إلى مجلد `d...
...
### 1.2 ضع أي توثيق تريد للبوت أن يجيب منه في مجلد `docs/` كملفات `.md`/`.txt` — ملف README لمشروع و...
...
شغّل الكود أدناه وتأكد من أنه يعمل.
...
- ✅ مجلد `docs/` موجود مع بضعة ملفات `.md`/`.txt` حقيقية على الأقل فيه.
- ✅ يعمل `uv run python build_index.py` دون أخطاء ويُبلِّغ عن عدد أجزاء غير صفري.
- ✅ `index.npy` و`chunks.json` موجودان الآن في مجلد مشروعك.
...
### 2.1 الاسترجاع أيضًا بلا تغيير من مشروع تطبيق RAG — ضمّن السؤال بنفس النموذج، ثم صنّف كل جزء بتشا...
...
```
إذا شعرت أن هذا سريع جدًا، فذلك متعمَّد — يغطي [مشروع تطبيق RAG](/ar/مشاريع/rag-notes#step-4-retrieve-relevant-chunks) بالضبط لماذا يعمل تشابه جيب التمام بهذه الطريقة، وماذا يمنحك التطبيع، وكيف ترتبط الرياضيات بضرب مصفوفة-متجه، بعمق أكبر بكثير مما ستضيفه إعادته هنا.
...
- ✅ يطبع `uv run python retrieve.py` نتائج مُصنَّفة بدرجات تشابه حقيقية.
- ✅ النتيجة الأولى لسؤال اختباري سهل تبدو ذات صلة فعليًا عند قراءتها.
- ✅ جرّبت سؤالًا واحدًا على الأقل لا يغطيه مجلد التوثيق الخاص بك بوضوح، وأكّدت أن الدرجة الأولى أقل بشكل ملحوظ.
...
### 3.1 هذا هو الجزء الجديد الفعلي من هذا المشروع: معالج أحداث `discord.py` يستدعي `retrieve()`، ويب...
...
- ✅ يُعرِّف `bot.py` كلًّا من `on_ready` و`on_message`، كلاهما كـ`async def`، كلاهما مُزيَّن بـ`@client.event`.
- ✅ يتحقق `on_message` من `message.author == client.user` قبل فعل أي شيء آخر.
- ✅ تستدعي `answer()` نفس `retrieve()` من الخطوة 2، دون تغيير.
...
### 4.1 عد إلى بوابة مطوري Discord، وافتح **OAuth2 → URL Generator**. تحت **Scopes**، حدّد `bot`؛ تح...
...
### 4.2 يجب أن ترى `Logged in as docs-qa-bot#1234 -- ready in 1 server(s).` مطبوعة — الصمت بعد ذلك ط...
...
- ✅ يظهر البوت متصلًا في قائمة أعضاء خادم اختبارك بعد تشغيل `uv run python bot.py`.
- ✅ ذكره بسؤال حقيقي ينتج مؤشر كتابة، ثم ردًّا.
- ✅ محتوى الرد يعكس فعليًا مجلد `docs/` الخاص بك، وسؤال لا تغطيه مستنداتك يحصل على "لا أعرف" صادقة بدلًا من تخمين واثق.
...
- بمجرد أن يفوق مجلد التوثيق الخاص بك ما يناسب الذاكرة بارتياح، انظر إلى قاعدة بيانات متجهية حقيقية مثل [ChromaDB](https://www.trychroma.com/)، تمامًا كما اقتُرح في ["إلى أين تذهب من هنا" الخاص بمشروع تطبيق RAG](/ar/مشاريع/rag-notes#where-to-go-from-here) — لا شيء بخصوص طبقة Discord يحتاج للتغيير لدعم ذلك.
