...
هذا يفترض إنهاء Python 101 والارتياح لكتابة دوال عادية — لا يُشترط أي شيء من تحليل البيانات. إنه اختياري وغير مُقيَّم؛ راجع [مشاريع من العالم الحقيقي](/ar/مشاريع) للاطلاع على القائمة الكاملة، والتي تنمو باستمرار. يتناسب هذا المشروع طبيعيًا مع [مشروع وكيل الذكاء الاصطناعي](/ar/مشاريع/ai-agent) — نفس الفكرة الجوهرية، منح الذكاء الاصطناعي أدوات يمكنه استدعاؤها، لكن من الجهة المقابلة: هناك بنيت الوكيل الذي يستدعي الأدوات مباشرة، في نفس عملية Python؛ هنا تبني خادمًا مستقلاً يستطيع *أي* عميل متوافق مع MCP الاتصال به، دون أن يحتاج ذلك العميل معرفة أي شيء عن كودك سوى البروتوكول.
...
ثم أعدّ مشروعًا وثبّت حزمة MCP الرسمية لـ Python، بإضافتها الاختيارية `cli` (هذا ما يمنحك أمر `mcp dev` المستخدم في الخطوة 2):
...
## الخطوة 1: اكتب أول خادم MCP خاص بك

واجهة الحزمة عالية المستوى، `FastMCP`، تحوّل دالة Python عادية إلى أداة MCP بمزخرف (decorator) واحد — لا كود على مستوى البروتوكول تكتبه يدويًا. أنشئ `server.py`:

### 1.1 اكتب الخادم

**👟 تلميح البداية :** أنشئ `FastMCP("course-tools")`، ثم اكتب دالتين Python عاديتين مع تلميحات الأنواع وسلسلة توثيق، كلٌّ منهما مزخرفة بـ `@mcp.tool()` — المزخرف هو ما يحوّل دالة عادية إلى شيء يستطيع عميل MCP اكتشافه واستدعاءه:
...
`@mcp.tool()` يقوم بكل عمل التسجيل هنا: يفحص اسم الدالة، ومعاملاتها المُنمَّطة (type-hinted)، وسلسلة توثيقها، ويبني منها تعريف أداة MCP تلقائيًا — لن تكتب مخططًا (schema) يدويًا أبدًا. هذه نفس الفكرة التي يعلّمها [مشروع وكيل الذكاء الاصطناعي](/ar/مشاريع/ai-agent) لأدوات LangChain: **النموذج يقرأ سلسلة توثيقك، لا كودك، ليقرر متى تناسب أداة ما طلبًا معينًا.** سلسلة توثيق غامضة لا تمنح النموذج شيئًا يعتمد عليه؛ سلسلة توثيق تقول بوضوح ماذا تفعل الأداة ومتى تُستدعى هي ما يجعل اختيار الأداة يعمل فعليًا.

`search_course_topics` هي عمدًا نفس فكرة الأداة التجريبية من مشروع وكيل الذكاء الاصطناعي — البحث في ملفات هذه الدورة نفسها عن موضوع — لكنها معروضة عبر مزخرف أداة MCP بدلاً من تمريرها مباشرة إلى قائمة `tools=[...]` لوكيل. `count_words` أداة مساعدة أصغر ومستقلة، أُدرجت لتوضيح خادم يعرض أكثر من أداة واحدة في آن واحد — يرى عميل MCP كلتيهما ويختار أيًّا منهما يناسب سؤالاً معينًا.

:::tip[تحقق من الوثائق الحالية لحزمة MCP قبل الاعتماد على هذا]
MCP مواصفة فتية وسريعة التطور — تغيّر البروتوكول نفسه، وواجهة حزمة Python الخاصة به، منذ الإصدارات الأولى. ظل أسلوب `FastMCP` القائم على المزخرفات مستقرًا لفترة، لكن قبل بناء أي شيء يتجاوز هذا الدرس، تصفّح [ملف README الخاص بالحزمة ووثائقها](https://github.com/modelcontextprotocol/python-sdk) بدلاً من افتراض أن تفاصيل هذا المقتطف لا تزال تطابق تمامًا.
:::

**🎯 الناتج المتوقع :** لا يوجد ناتج بعد — `server.py` بمفرده يُعرِّف فقط الأداتين؛ لا يوجد شيء لتشغيله حتى الخطوة 2. الإشارة في هذه المرحلة هي أنه *يُستورد* بشكل سليم: `uv run python -c "import server"` يجب أن لا يُنتج أي تتبّع خطأ.

**🩹 إذا لم يعمل :** `ModuleNotFoundError: No module named 'mcp'` يعني أن `uv add "mcp[cli]"` من الإعداد لم يعمل في مجلد هذا المشروع — تأكد أنك داخل مجلد `mcp-server` الذي أنشأه `uv init`. إذا لم يكن `DOCS_DIR` يشير إلى مجلد فعلي على جهازك، فلن ترتكب `search_course_topics` خطأً بعد (يظهر ذلك فقط عندما تستدعيها فعلًا في الخطوة 2) — أصلح المسار الآن بينما هو طازج.

### 1.2 تحقق من أنه يُستورد بشكل سليم

**✅ قائمة التحقق**

- ✅ يُحفظ `server.py` دون أخطاء صياغية ويُعرِّف كلًّا من `search_course_topics` و`count_words`.
- ✅ لكل أداة سلسلة توثيق حقيقية بلغة إنجليزية واضحة — لا عنصر نائب (placeholder).
- ✅ يشير `DOCS_DIR` إلى مجلد `docs/` حقيقي موجود فعلاً على جهازك.
...
- تُعيد `search_course_topics` سلسلة نصية، لا بيانات مهيكلة. ماذا قد تخسر، أو تكسب، لو أعادت قائمة من التطابقات بدلاً من ذلك؟

## الخطوة 2: شغّل واختبر خادمك محليًا
...
### 2.1 ابدأ الـ Inspector

**👟 تلميح البداية :** شغّل أمر التطوير أدناه، واتركه يفتح علامة متصفح، واستدعِ كل أداة يدويًا من هناك قبل أن تقترب من Claude Desktop:
...
**🎯 الناتج المتوقع :** تُظهر قائمة أدوات Inspector `search_course_topics` و`count_words` بنماذج معاملاتها المُولَّدة تلقائيًا من تلميحات أنواعك. استدعاء `search_course_topics` بـ `"groupby"` يُعيد سلسلة `Found in: ...` حقيقية بأسماء ملفات مطابقة؛ استدعاء `count_words` بجملة قصيرة يُعيد العدد الصحيح الصحيح.

**🩹 إذا لم يعمل :** إذا أعادت `search_course_topics` `No lesson pages mention '...'` لاستعلام تعرف أنه مغطّى، فإن `DOCS_DIR` يكاد بالتأكيد ما يزال يشير إلى مسار العنصر النائب من الخطوة 1 — أصلحه وأعد تشغيل خادم التطوير (لا يعيد التحميل الساخن). إذا لم تُفتح علامة Inspector أبدًا، تحقق من الطرفية بحثًا عن طلب تثبيت `npx` قد تنتظره.

### 2.2 استدعِ كلتا الأداتين يدويًا

**✅ قائمة التحقق**

- ✅ يبدأ `uv run mcp dev server.py` دون أخطاء ويفتح Inspector في متصفحك.
- ✅ يُدرج Inspector كلًّا من `search_course_topics` و`count_words`.
- ✅ استدعاء كل أداة يدويًا في Inspector يُعيد نتيجة حقيقية وصحيحة — لا خطأ.
...
## الخطوة 3: اربطه بـ Claude Desktop
...
### 3.1 عدّل ملف الإعداد

**👟 تلميح البداية :** انسخ JSON أدناه إلى ذلك الملف، ثم استبدل `/absolute/path/to/mcp-server` بالمسار الكامل الحقيقي لمجلد مشروعك — `pwd` (macOS/Linux) أو `cd` دون معاملات (Windows) من داخله سيطبع ذلك بالضبط:
...
**🎯 الناتج المتوقع :** كتلة مرئية بعنوان "استخدم أداة" في رد Claude Desktop تُظهر أن `search_course_topics` استُدعيت، بنفس النتيجة التي رأيتها بالفعل في Inspector، يليها رد Claude المبني من تلك النتيجة.

**🩹 إذا لم يعمل :** إذا لم يظهر `course-tools` في قائمة الموصِّلات أبدًا، فإما أن JSON في ملف الإعداد به خطأ صياغة (الفاصلة اللاحقة هي الخطأ الكلاسيكي) أو أنك تخطيت إعادة التشغيل *الكاملة* — إغلاق النافذة وحده لا يعيد تحميله. إذا ظهر لكن Claude أجاب دون استدعاء الأداة، حاول إعادة صياغة السؤال للإشارة بشكل أكثر صراحةً إلى "بحث course-tools" بنفس الطريقة التي يفعلها سؤال المثال — يقرر النموذج من تلقاء نفسه ما إذا كانت الأداة ذات صلة، والسؤال الغامض يعطيه سببًا أقل.

### 3.2 أعد تشغيل Claude Desktop وتحقق أنه يستدعي أداتك

**✅ قائمة التحقق**

- ✅ يظهر `course-tools` (أو اسم الخادم الذي اخترته) في قائمة أدوات/موصِّلات Claude Desktop بعد إعادة تشغيل كاملة.
- ✅ طرح سؤال يجب أن يُحفّز `search_course_topics` يُظهر فعلاً أن Claude يستدعيها، لا مجرد الإجابة من الذاكرة.
- ✅ النتيجة التي يعرضها Claude تطابق ما رأيته أثناء اختبار نفس الاستدعاء في Inspector.
...
- اقرأ عن **الموارد** (resources) و**المُوجِّهات** (prompts) في MCP — يغطي هذا الدرس *الأدوات* فقط، لكن البروتوكول يُعرِّف أيضًا طرقًا لعرض بيانات قابلة للقراءة (الموارد) وقوالب موجِّهات قابلة لإعادة الاستخدام (المُوجِّهات) لعميل ما. تغطي [وثائق الحزمة نفسها](https://github.com/modelcontextprotocol/python-sdk) كليهما، بنفس أسلوب مزخرف `FastMCP`.
