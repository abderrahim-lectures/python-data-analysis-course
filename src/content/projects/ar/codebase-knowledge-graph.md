...
هذا يفترض Python 101 وارتياحًا مع الدوال والاستيرادات — لا يُشترط أي شيء من تحليل البيانات، ولا شيء هنا يستدعي أي نموذج ذكاء اصطناعي أو خدمة ويب. هذا اختياري وغير مُقيَّم؛ راجع [مشاريع من العالم الحقيقي](/ar/مشاريع) للاطلاع على القائمة الكاملة والنامية.
...
### 1.1 قبل تحليل مستودع كامل، اجعل ملفًا واحدًا يعمل. وحدة `ast` المدمجة في Python تحوّل الكود المص...
...
### 1.2 ثم اكتب `explore_ast.py` لاستكشافه:
...
### 2.1 بنية ملف واحد بداية؛ قيمة مستودع كامل من الملفات، والدوال، والفئات، وعلاقاتها هي ما يجعل هذا...
...
### 3.1 الملفات والدوال والفئات والاستيرادات تصف ما *موجود*. لالتقاط كيفية *استخدام* الأجزاء لبعضها ...
...
### 3.2 `node.func` في `ast.Call` إما `ast.Name` (استدعاء بسيط مثل `add(...)`) أو `ast.Attribute` (ا...
...
### 4.1 رسم بياني بضع مئات من العُقَد غير قابل للقراءة كقائمة أضلاع — تصوره هو ما يتيح لك فعليًا *رؤ...
...
### 4.3 افتح `graph.html` الناتج في متصفح. العُقَد مُلوَّنة حسب النوع (ملفات زرقاء، فئات كهرمانية، د...
...
### 5.1 رسم بياني يمكنك فقط النظر إليه مفيد بالفعل، لكن رسمًا بيانيًا يمكنك *سؤاله* أكثر فائدة — وبم...
...
### 6.1 كل شيء حتى الآن كان يبني نحو شيء واحد: توجيه الأداة المكتملة إلى قاعدة كود لم يبنها أحد خصيص...
...
### 6.2 بمجرد أن يعمل ذلك، وجّهه إلى شيء حقيقي — **مستودع هذه الدورة نفسه قاعدة كود Python حقيقية وغ...
