...
# 🕸️ حوّل قاعدة كود إلى رسم بياني معرفي
...
- ✅ يعمل `uv run python explore_ast.py` دون أخطاء ويطبع `function: greet` و`class: Greeter` و`import: os`.
- ✅ تُطبَع أيضًا `function: greet_twice`، رغم أنها متداخلة داخل `Greeter` — مؤكدًا أن `ast.walk` يزور كل عقدة، لا عُقَد المستوى الأعلى فقط.
- ✅ يمكنك أن تشرح، في جملة واحدة، الفرق بين `ast.Import` (`import os`) و`ast.ImportFrom` (`from x import y`).
...
- ✅ تشغيل `build_graph.py` مقابل مجلد صغير من ملفات `.py` يطبع عدد عُقَد وأضلاع غير صفري.
- ✅ ملف يُعرِّف دالتين ويستورد وحدة واحدة ينتج على الأقل 4 عُقَد لهذا الملف وحده (الملف نفسه، والوحدة، والدالتان).
- ✅ اكسر عمدًا صيغة ملف واحد (قوس غير مُغلَق) وتأكد أن الأداة تتخطاه بتحذير بدلًا من التعطل.
...
- ✅ بعد تشغيل الأداة الكاملة على `sample_repo/` (من المثال المصاحب، أو ملفات اختبارك الخاصة)، يوجد ضلع `"calls"` واحد على الأقل بين دالتين في ملفين مختلفين.
- ✅ يمكنك الإشارة إلى استدعاء محدد في كود اختبارك وإيجاد الضلع المطابق في الرسم البياني.
- ✅ يمكنك أن تشرح لماذا يجب أن تعمل خطوة حل الاستدعاءات *بعد* مسح كل ملف، لا ملفًا بملف أثناء التقدم.
...
- ✅ يفتح `graph.html` في متصفح ويُظهر رسمًا بيانيًا حقيقيًا وغير فارغ — لا صفحة فارغة.
- ✅ سحب عقدة يحرّكها، وتتبعها الأضلاع المتصلة.
- ✅ تمرير الفأرة فوق عقدة يُظهر نوعها ومُعرِّفها الكامل في تلميح.
- ✅ (إذا جربت نسخة matplotlib) يوجد `graph.png` ويُفتح كصورة حقيقية، بألوان عُقَد مميَّزة.
...
- ✅ `what_does_it_call(graph, ...)` على دالة تعرف أنها تستدعي دالتين أخريين يُعيد كلتيهما، بالاسم.
- ✅ `who_imports(graph, ...)` على وحدة تعرف أن ملفين يستورداها يُعيد اسمي الملفين.
- ✅ الاستعلام عن اسم غير موجود في الرسم البياني يُعيد نتيجة فارغة، لا تعطلًا.
...
- ✅ تعمل الأداة مقابل مستودع حقيقي متعدد الملفات (لا `sample_repo/` التجريبي فقط) دون تعطل.
- ✅ الرسم البياني الناتج لديه عُقَد وأضلاع أكثر بوضوح من المثال التجريبي، والتصور لا يزال يُصيَّر.
- ✅ يمكنك تسمية شيء واحد أظهره لك الرسم البياني عن بنية تلك قاعدة الكود لم تكن تعرفه مسبقًا.
