...
- ✅ يطبع 10 أسطر بالضبط، سطرًا واحدًا لكل اقتباس في الصفحة الرئيسية.
- ✅ كل سطر مطبوع يحتوي نصًا حقيقيًا، واسم مؤلف حقيقيًا، وقائمة وسوم غير فارغة — لا `None` أو سلاسل فارغة.
...
- ✅ ينتهي `uv run python scrape.py` ويطبع سطر "Saved N quotes".
- ✅ موجود ملف `quotes.csv` ويحتوي أكثر من 10 صفوف (أي أنه تابع ترقيم الصفحات فعلاً، لا الصفحة الرئيسية فقط).
- ✅ فتح `quotes.csv` في محرر نصوص يُظهر ثلاثة أعمدة — `text` و`author` و`tags` — دون صفوف مكسورة/فارغة واضحة.
...
- ✅ يحتوي `df["tags"]` قوائم Python حقيقية بعد استدعاء `.apply`، لا سلاسل نصية — تحقق بـ `type(df["tags"].iloc[0])`.
...
- ✅ موجود كل من `top_tags.png` و`quote_length_dist.png` ويُفتحان كصورتين حقيقيتين.
- ✅ محور x للرسم الشريطي يبدأ من 0.
- ✅ لكلا الرسمين عنوان ومحاور مُصنَّفة — لا أرقام مجردة دون وحدات.
