...
### 2.1 يستهدف هذا المشروع [quotes.toscrape.com](https://quotes.toscrape.com) — موقع عام بُني ويُصان...
...
### 2.2 تلك السلسلة النصية `html` هي شجرة من وسوم متداخلة — `<div>`، `<span>`، `<a>` — كل واحدة تحمل...
...
### 3.1 يوزّع quotes.toscrape.com اقتباساته عبر عدة صفحات، مع رابط "التالي" في أسفل كل صفحة عدا الأخ...
...
### 4.1 قدّم قسم تحليل البيانات pandas كحل لبطء حلقات Python العادية مع نمو البيانات — عمليات مُتَّج...
...
### 5.1 بأعمدة نظيفة ومُنمَّطة، التحليل الفعلي هو بضعة أسطر من `groupby`/`value_counts`، تمامًا كدفا...
...
### 5.3 **توزيع طول الاقتباسات** — رسم بياني تكراري (histogram)، لرؤية شكل البيانات لا مجرد متوسط واحد:
