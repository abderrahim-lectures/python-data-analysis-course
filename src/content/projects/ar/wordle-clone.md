...
- **Google Colab وKaggle Notebooks أو Binder.** لا يحتاج هذا المشروع إلى أي تبعيات خارجية، ما يجعله مناسبًا ممتازًا لدفتر الملاحظات بمعنى ما — لكن موجه `input()` في دفتر الملاحظات مختلف قليلًا عن طرفية تفاعلية حقيقية: لا إعادة رسم للبلاطات الملونة في مكانها على سطر واحد، و(على Colab/Kaggle) لا تنجو الملفات المحلية للجلسة بشكل موثوق بين زيارات منفصلة، وهو ما يضرب ضد جزء "تستمر الإحصائيات عبر الجلسات" من هذا المشروع. يظل [`notebook.ar.ipynb`](https://github.com/abderrahim-lectures/python-data-analysis-course/blob/main/examples/wordle-clone/notebook.ar.ipynb) نسخة حقيقية قابلة للعب — تستحق التجربة — فقط اعلم أن التجربة الكاملة (البلاطات الملونة في الطرفية، والإحصائيات التي تستمر بين أيام لعب منفصلة) هي فعليًا شيء "شغّله محليًا".

  [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/abderrahim-lectures/python-data-analysis-course/blob/main/examples/wordle-clone/notebook.ar.ipynb)
  [![Open In Kaggle](https://kaggle.com/static/images/open-in-kaggle.svg)](https://kaggle.com/kernels/welcome?src=https://github.com/abderrahim-lectures/python-data-analysis-course/blob/main/examples/wordle-clone/notebook.ar.ipynb)
  [![Binder](https://mybinder.org/badge_logo.svg)](https://mybinder.org/v2/gh/abderrahim-lectures/python-data-analysis-course/main?filepath=examples%2Fwordle-clone%2Fnotebook.ar.ipynb)
...
### 1.1 ابدأ بالجزء الذي من السهل أن يُتقَن *تقريبًا* ومن المُرضي أن يُتقَن *فعليًا*: بوجود تخمين من...
...
### 1.2 جرّبه على `guess = "SPEED"`, `target = "ERASE"`. تحتوي الكلمة المستهدفة على **حرف** `E` واحد...
...
### 1.3 يميّز التمرير الأول كل تطابق في الموضع الصحيح بالأخضر، ويُحصي بشكل منفصل (في `remaining`) كم...
...
### 2.1 بعد إتقان التسجيل، لفِّه في لعبة فعلية: اختر هدفًا عشوائيًا من قائمة كلمات، وامنح اللاعب 6 ت...
...
### 3.1 لا يسمح لك Wordle الحقيقي بتخمين `"ZZZZZ"` — كل تخمين يجب أن يكون كلمة حقيقية من قاموسه. أضف...
...
### 4.1 القطعة الأخيرة: تذكّر أداء اللاعب عبر تشغيلات منفصلة للبرنامج، لا فقط داخل جلسة واحدة. ذلك ي...
...
### 4.2 يعالج `load_stats` التشغيل الأول برشاقة — لا يوجد ملف بعد، لذا يعيد مجموعة افتراضيات جديدة م...
