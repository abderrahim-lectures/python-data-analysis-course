...
يفترض هذا المشروع إتمام بايثون 101، ويُفضَّل أيضًا تحليل البيانات (الارتياح للجداول والأعمدة والاستعلام عن البيانات المهيكلة سيجعل جانب SQL يترسخ أسرع)، وأن تكون قد بنيت بالفعل مشروع [بناء خادم MCP](/ar/مشاريع/mcp-server) — يعيد هذا المشروع استخدام إعداد `FastMCP` من ذلك المشروع ولا يشرحه من جديد من الصفر. إنه اختياري وغير مُقيَّم؛ راجع [مشاريع من الواقع](/ar/مشاريع) للاطلاع على القائمة الكاملة والمتنامية.
...
### 1.1 أنشئ `seed.py` — سكربت يبني قاعدة بيانات مكتبة صغيرة بأربعة جداول مترابطة:
...
- ✅ يعمل `uv run python seed.py` دون أخطاء وينشئ `library.db`.
- ✅ تحتوي قاعدة البيانات على ثلاثة جداول مترابطة على الأقل، متصلة بمفاتيح خارجية (وليست جدولًا مسطحًا واحدًا).
- ✅ يحتوي صف واحد على الأقل على قيمة `NULL` في عمود قابل لذلك (مثل إعارة لم تُعَد) — البيانات الحقيقية تحتوي فجوات.
...
### 2.1 أنشئ `db_tools.py` — دوال بايثون بسيطة، دون أي استيراد لـ `mcp` على الإطلاق، سيغلّفها الخادم...
...
- ✅ لا يحتوي `db_tools.py` على أي `import` لـ `mcp` في أي مكان — إنه `sqlite3` والمكتبة القياسية فقط.
- ✅ يطلق `run_read_only_query("DROP TABLE books")` استثناء `UnsafeQueryError` بدلاً من التنفيذ.
- ✅ يطلق `run_read_only_query("SELECT * FROM books; DROP TABLE books")` أيضًا استثناء `UnsafeQueryError` — يكتشف فحص الفاصلة المنقوطة العبارات المسلسلة.
- ✅ يُعيد استعلام `SELECT` حقيقي على قاعدة بياناتك الصفوف الصحيحة كقائمة من القواميس.
...
### 3.1 أنشئ `server.py`، مستوردًا الدوال من الخطوة 2 ومغلّفًا كل واحدة بـ `@mcp.tool()`، تمامًا مثل...
...
### 3.2 اختبره تمامًا كما في مشروع MCP السابق، باستخدام Inspector، قبل لمس أي عميل حقيقي:
...
- ✅ يبدأ `uv run mcp dev server.py` بسلاسة ويسرد Inspector الأدوات الثلاث جميعها.
- ✅ تُعيد كل من `list_db_tables` و`describe_db_table` بيانات حقيقية وصحيحة في Inspector.
- ✅ يُعيد `query_db` مع استعلام `SELECT` حقيقي صفوفًا؛ ويُعيد `query_db` مع استعلام كتابة/DDL نتيجة `{"error": ...}` واضحة بدلاً من الانهيار.
...
### 4.1 أضف خادمك إلى `claude_desktop_config.json` (نفس الملف الذي استخدمه مشروع بناء خادم MCP؛ macO...
...
- ✅ يظهر `library-db` في قائمة أدوات Claude Desktop بعد إعادة تشغيل كاملة.
- ✅ يُظهر طرح السؤال النموذجي أعلاه Claude يستدعي فعليًا `list_db_tables` و`describe_db_table` و`query_db` بالتسلسل، لا مجرد الإجابة من الذاكرة.
- ✅ SQL الذي كتبه Claude (الظاهر في تفاصيل استدعاء الأداة الموسّعة) هو ربط (join) حقيقي متعدد الجداول، والإجابة تطابق ما ستحصل عليه من تشغيل ذلك الاستعلام بنفسك.
