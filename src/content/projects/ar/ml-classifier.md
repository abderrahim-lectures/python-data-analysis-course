...
### 2.1 نفس المجموعة، نفس أعمدة التحليل الاستكشافي في الأسبوع 10 — هذه المرة مُحمَّلة من ملف مجموعة ...
...
### 2.2 ملخّص سريع للتنظيف الذي شرحه الأسبوع 10 بالتفصيل بالفعل — فقط بالقدر الكافي هنا للحصول على D...
...
### 2.4 يُسقِط `drop_first=True` فئة واحدة لكل عمود (مثلًا يبقي `Sex_male` لكن لا يبقي `Sex_female`)...
...
- ✅ يُظهر `df.isna().sum()` صفر قيم مفقودة في كل عمود توشك على تغذيته للنموذج.
- ✅ لا تُظهر `X.dtypes` أي أعمدة `object` متبقية — كل شيء رقمي.
- ✅ لا يحتوي `X` عمود `Survived`؛ ولا يحتوي `y` أي شيء آخر.
...
### 3.1 إليك الفكرة الجوهرية التي تُبنى عليها هذه الخطوة: **درجة نموذج على البيانات التي دُرِّب عليه...
...
- ✅ يُظهر `X_train.shape` و`X_test.shape` تقسيمًا تقريبيًا 80/20 من إجمالي عدد الصفوف.
- ✅ إعادة تشغيل التقسيم بنفس `random_state` يُعيد إنتاج نفس الصفوف بالضبط في `X_test` في كل مرة.
- ✅ كل من `y_train` و`y_test` مزيج من 0 و1، لا قيمة واحدة فقط.
...
### 4.1 `LogisticRegression`، رغم اسمها، هي مصنِّف، لا نموذج انحدار بالمعنى المعتاد. الفكرة: لكل راك...
...
- ✅ يعمل `model.fit(...)` دون تحذير عدم تقارب (أو رفعت `max_iter` حتى لا يظهر).
- ✅ `predictions` مصفوفة بنفس طول `y_test`، تحتوي فقط 0 و1.
- ✅ يمكنك طباعة `model.predict_proba(X_test)[:5]` ورؤيتها تُعيد احتمالات فعلية، لا مجرد القرار النهائي 0/1.
...
### 5.1 الرقم الوحيد للبدء به هو الدقة (accuracy) — نسبة تنبؤات مجموعة الاختبار التي طابقت النتيجة ا...
...
### 5.2 تخفي الدقة وحدها *نوع* الأخطاء التي يرتكبها النموذج. تفصّل مصفوفة الالتباس (confusion matrix...
...
### 5.3 النتيجة شبكة 2×2. بقراءتها بعبارات بسيطة: تعدّ، بشكل منفصل، كم راكبًا مات فعلاً وتنبَّأ النم...
...
- ✅ لديك رقما دقة، محسوبان على *نفس* `X_test`/`y_test`، واحد لكل نموذج.
- ✅ طبعت مصفوفتَي الالتباس لكلتيهما وتستطيع القول، بجملة واحدة، أي أنواع الأخطاء ارتكبها كل نموذج.
- ✅ لم تُعلن "فائزًا" دون النظر إلى مدى ضآلة الفجوة بينهما فعليًا.
