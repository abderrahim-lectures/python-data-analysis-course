...
- ✅ `df.isna().sum()` muestra cero valores faltantes en cada columna que estás a punto de entregarle al modelo.
- ✅ `X.dtypes` no muestra ninguna columna `object` restante — todo es numérico.
- ✅ `X` no contiene la columna `Survived`; `y` no contiene nada más.
...
- ✅ `X_train.shape` y `X_test.shape` muestran aproximadamente una división 80/20 del total de filas.
- ✅ Volver a ejecutar la división con el mismo `random_state` reproduce exactamente las mismas filas en `X_test` cada vez.
- ✅ `y_train` y `y_test` son ambos una mezcla de 0s y 1s, no todo un solo valor.
...
- ✅ `model.fit(...)` se ejecuta sin advertencia de convergencia (o subiste `max_iter` hasta que dejó de aparecer).
- ✅ `predictions` es un array de la misma longitud que `y_test`, conteniendo solo 0s y 1s.
- ✅ Puedes imprimir `model.predict_proba(X_test)[:5]` y ver que devuelve probabilidades reales, no solo la decisión final de 0/1.
...
- ✅ Tienes dos números de precisión, calculados sobre el *mismo* `X_test`/`y_test`, uno por modelo.
- ✅ Has impreso ambas matrices de confusión y puedes decir, en una oración, qué tipo de errores cometió cada modelo.
- ✅ No has declarado un "ganador" sin considerar qué tan pequeña es realmente la brecha entre ellos.
