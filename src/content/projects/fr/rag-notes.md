...
- ✅ `uv run python prepare_notes.py` s'exécute sans erreur et affiche un nombre de fragments non nul.
- ✅ Les aperçus affichés ressemblent à de vrais fragments de vos notes, pas à des chaînes vides ou d'énormes blocs de texte fusionné.
- ✅ `NOTES_DIR` pointe vers un dossier qui contient bien des fichiers `.md`/`.txt`.
...
- ✅ Le premier nombre de la forme affichée correspond au nombre de fragments de l'étape 2, et le second nombre est 384.
...
- ✅ Le fragment le mieux classé pour une question de test simple et évidente semble effectivement pertinent en le lisant.
- ✅ Les scores sont entre -1 et 1 (la plage valide pour la similarité cosinus) — si vous voyez des nombres bien en dehors, un des vecteurs n'a probablement pas été normalisé.
...
- ✅ `uv run python ask.py "une vraie question sur vos notes"` affiche une réponse, pas une trace d'erreur.
- ✅ La réponse reflète effectivement le contenu de vos notes, pas une connaissance générique que le modèle avait déjà.
- ✅ Demander quelque chose que vos notes ne couvrent clairement pas fait dire au modèle qu'il ne sait pas, plutôt que d'inventer quelque chose avec assurance.
