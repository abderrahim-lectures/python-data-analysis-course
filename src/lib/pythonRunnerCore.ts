// Engine-agnostic Pyodide glue shared by the main-thread runner
// (runnable-cell.client.ts, used only for the handful of cells that need a
// synchronous window.prompt() for input()) and the Web Worker runner
// (pyodide.worker.ts, used for everything else so a slow cell doesn't block
// the page). Nothing here may assume `window` or `document` exist — a Worker
// has neither. The one exception is `stdin`, which is deliberately made the
// caller's responsibility (see CellRuntime.stdin) rather than defaulted to
// `window.prompt` here, so this module stays usable from either context.
...
export const INDEX = `${import.meta.env.BASE_URL}pyodide/`;
...
// Unit tests reset the per-install cache between cases (it is per-page-load /
// per-worker-lifetime in the browser, but module-scoped in a single vitest
// process).
...
// the virtual filesystem used to pre-mount datasets).
...
// browser reclaims storage under real pressure. This patches fetch once,
// permanently, for the page's (or worker's) lifetime -- not just the initial
// engine boot -- so the same origin-hosted package wheels a later lesson
// pulls in via loadPackagesFromImports/micropip are cache-first too, not
// just the core runtime. `caches`/`fetch` both exist in a Worker's global
// scope the same as in a window, so this needs no worker-specific branching.
...
// Loads (or returns the already-loading/loaded) Pyodide engine. `onProgress`
// is called with 0-99 while the core runtime downloads (skipped once cached
// -- see ensurePyodideCache); callers report readiness themselves once this
// resolves. Kept caller-supplied rather than dispatched here directly so
// both the main-thread path (window.dispatchEvent) and the worker path
// (postMessage) can report progress their own way without this module
// needing to know which context it's running in.
export async function py(onProgress?: (pct: number) => void): Promise<PyodideModel> {
  if (!pyPromise) {
    pyPromise = (async () => {
      ensurePyodideCache();
      const coreBytes = await getCoreBytes();
      let loaded = 0;
      if (coreBytes > 0 && onProgress) {
        onPyodideBytes = (n) => {
          loaded += n;
...
// are cached so each file is fetched once per page load (or per worker
// lifetime).
...
// Unit tests reset the per-mount cache between cases.
export function resetMountedDatasetsCache(): void {
  mountedDatasets.clear();
}

const mountedDatasets = new Set<string>();
// `allSource` is every runnable cell's code on the page, concatenated by the
// caller -- only the DOM-owning main thread can walk the page for that, so
// unlike the rest of this module it can't gather that itself (a Worker has
// no document to query).
export async function mountDatasets(engine: PyodideModel, allSource: string): Promise<void> {
  const manifest = await getDatasetManifest();
  if (!manifest) return;
  for (const {ref, shipped} of planDatasetMounts(allSource, manifest)) {
...
export interface CellRuntime {
  ...
  /** Supplies `input()`'s return value. Defaults to `window.prompt` when
   *  omitted and `window` exists (the main-thread path); a Worker has no
   *  `window`; cells that call `input()` never reach the worker (see
   *  usesBlockingInput), so its stdin is never actually exercised. */
  stdin?(): string;
}
...
export async function runCellCode(code: string, rt: CellRuntime): Promise<boolean> {
  ...
  rt.engine.setStdin({
    stdin: () => {
      if (rt.stdin) return rt.stdin();
      return typeof window !== 'undefined' && typeof window.prompt === 'function' ? (window.prompt('') ?? '') : '';
    },
  });
